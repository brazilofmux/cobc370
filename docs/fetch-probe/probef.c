...
char probe_big[262144];

int probe(int mode)
{
    volatile unsigned char *p = (volatile unsigned char *)probe_big;
    int i;
    if (mode) { for (i = 0; i < 262144; i++) p[i] = 0xFF; return -1; }
    for (i = 0; i < 262144; i++) if (p[i]) return i + 1;
    return 0;
}
