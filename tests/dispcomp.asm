*---------------------------------------------------------------
* Generated by cobc370 from dispcomp.cbl
...
* DISPLAY
         ...
         UNPK  ZWK(4),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+3,X'10'         a D (or B) zone is negative
         BO    L0004               keep it overpunched
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0004    DS    0H
         MVC   DSPBUF+6(4),ZWK+0
...
* DISPLAY
         ...
         BO    L0005               keep it overpunched
         OI    ZWK+8,X'F0'         otherwise a plain digit
L0005    DS    0H
         MVC   DSPBUF+10(9),ZWK+0
         MVC   DSPBUF+19(1),S0002
         LA    1,PARM0005
         L     15,VDISP
         BALR  14,15
T0010    DS    0H
...
* DISPLAY
         ...
         UNPK  ZWK(4),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+3,X'10'         a D (or B) zone is negative
         BO    L0007               keep it overpunched
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0007    DS    0H
         MVC   DSPBUF+12(4),ZWK+0
         MVC   DSPBUF+16(1),S0002
         LA    1,PARM0007
         L     15,VDISP
         BALR  14,15
T0014    DS    0H
...
* DISPLAY
         ...
         UNPK  ZWK(5),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+4,X'10'         a D (or B) zone is negative
         BO    L0010               keep it overpunched
         OI    ZWK+4,X'F0'         otherwise a plain digit
L0010    DS    0H
         MVC   DSPBUF+6(5),ZWK+0
...
* DISPLAY
         ...
         OI    ZWK+4,X'F0'         otherwise a plain digit
L0011    DS    0H
         MVC   DSPBUF+12(5),ZWK+0
         MVC   DSPBUF+17(1),S0002
         LA    1,PARM0011
         L     15,VDISP
         BALR  14,15
T0022    DS    0H
...
* DISPLAY
         ...
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0013    DS    0H
         MVC   DSPBUF+4(4),ZWK+0
         MVC   DSPBUF+8(5),S0017
         ZAP   PWK1(16),D0004(3)
         UNPK  ZWK(5),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+4,X'10'         a D (or B) zone is negative
...
* DISPLAY
         ...
         ZAP   PWK1(16),D0009(10)
         UNPK  ZWK+3(15),PWK1+8(8)  DISPLAY: the low fifteen digits, zo
         MVI   PWK2+8,X'0F'        a sign for the high digits
         MVO   PWK2(9),PWK1(8)     the high digits, a nibble right
         UNPK  ZWK(3),PWK2(9)      and zoned in front
         TM    ZWK+17,X'10'        a D (or B) zone is negative
         BO    L0015               keep it overpunched
         OI    ZWK+17,X'F0'        otherwise a plain digit
L0015    DS    0H
         MVC   DSPBUF+11(18),ZWK+0
         MVC   DSPBUF+29(1),S0002
         LA    1,PARM0016
         L     15,VDISP
         BALR  14,15
T0031    DS    0H
...
* DISPLAY
         ...
         UNPK  ZWK(4),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+3,X'10'         a D (or B) zone is negative
         BO    L0017               keep it overpunched
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0017    DS    0H
         MVC   DSPBUF+7(4),ZWK+0
         MVC   DSPBUF+11(2),S0021
         PACK  DWK(8),D0012(1)     subscript
         CVB   7,DWK
         ...
         UNPK  ZWK(4),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+3,X'10'         a D (or B) zone is negative
         BO    L0018               keep it overpunched
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0018    DS    0H
         MVC   DSPBUF+13(4),ZWK+0
         MVC   DSPBUF+17(2),S0021
         LA    7,2                 subscript-1
         MH    7,H0001             times element size
         LA    7,D0011(7)          element address
         LH    2,0(,7)
         ...
         UNPK  ZWK(4),PWK1(16)     DISPLAY: the digits, zoned
         TM    ZWK+3,X'10'         a D (or B) zone is negative
         BO    L0019               keep it overpunched
         OI    ZWK+3,X'F0'         otherwise a plain digit
L0019    DS    0H
         MVC   DSPBUF+19(4),ZWK+0
         MVC   DSPBUF+23(1),S0002
         LA    1,PARM0018
         L     15,VDISP
         BALR  14,15
T0037    DS    0H
...
WK4      DS    PL16
WK5      DS    PL16
K0001    DC    PL16'12'            numeric constants
K0002    DC    PL16'-12'
K0003    DC    PL16'0'
K0004    DC    PL16'1234'
K0005    DC    PL16'-123456789'
K0006    DC    PL16'987654321'
K0007    DC    PL16'-1234'
K0008    DC    PL16'12345'
K0009    DC    PL16'-12345'
K0010    DC    PL16'543'
...
K0012    DC    PL16'-7'
K0013    DC    PL16'7'
K0014    DC    PL16'-123456789012345678'
K0015    DC    PL16'1'
K0016    DC    PL16'11'
K0017    DC    PL16'-22'
K0018    DC    PL16'33'
...
S0004    DC    CL11'H-S4 ZERO ['
S0005    DC    CL6'H-U4 ['
S0006    DC    CL10'F-S9 NEG ['
S0007    DC    CL10'F-S9 POS ['
S0008    DC    CL12'H-S2V2 NEG ['
S0009    DC    CL10'P-S5 POS ['
S0010    DC    CL10'P-S5 NEG ['
S0011    DC    CL6'P-U5 ['
S0012    DC    CL12'P-S3V2 NEG ['
S0013    DC    CL10'P-S4 NEG ['
S0014    DC    CL10'Z-S4 NEG ['
S0015    DC    CL10'Z-S4 POS ['
S0016    DC    CL4'TWO '
S0017    DC    CL5' AND '
S0018    DC    CL11'P-S18 NEG ['
S0019    DC    CL11'P-S18 ONE ['
S0020    DC    CL7'TABLE ['
S0021    DC    CL2']['
...
SPIENUM  DC    H'38'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0H
         ...
         DC    AL2(T0006-COBBEG),AL2(30)
         DC    AL2(T0007-COBBEG),AL2(31)
         DC    AL2(T0008-COBBEG),AL2(32)
         DC    AL2(T0009-COBBEG),AL2(33)
         DC    AL2(T0010-COBBEG),AL2(34)
         DC    AL2(T0011-COBBEG),AL2(35)
         DC    AL2(T0012-COBBEG),AL2(36)
         DC    AL2(T0013-COBBEG),AL2(37)
         DC    AL2(T0014-COBBEG),AL2(38)
         DC    AL2(T0015-COBBEG),AL2(39)
         DC    AL2(T0016-COBBEG),AL2(40)
         DC    AL2(T0017-COBBEG),AL2(41)
         DC    AL2(T0018-COBBEG),AL2(42)
         DC    AL2(T0019-COBBEG),AL2(43)
         DC    AL2(T0020-COBBEG),AL2(44)
         DC    AL2(T0021-COBBEG),AL2(45)
         DC    AL2(T0022-COBBEG),AL2(46)
         DC    AL2(T0023-COBBEG),AL2(47)
         DC    AL2(T0024-COBBEG),AL2(48)
         DC    AL2(T0025-COBBEG),AL2(49)
         DC    AL2(T0026-COBBEG),AL2(50)
         DC    AL2(T0027-COBBEG),AL2(51)
         DC    AL2(T0028-COBBEG),AL2(52)
         DC    AL2(T0029-COBBEG),AL2(53)
         DC    AL2(T0030-COBBEG),AL2(54)
         DC    AL2(T0031-COBBEG),AL2(55)
         DC    AL2(T0032-COBBEG),AL2(56)
         DC    AL2(T0033-COBBEG),AL2(57)
         DC    AL2(T0034-COBBEG),AL2(57)
         DC    AL2(T0035-COBBEG),AL2(57)
         DC    AL2(T0036-COBBEG),AL2(58)
         DC    AL2(T0037-COBBEG),AL2(59)
COBWS    CSECT
WSC0000  EQU   COBWS               chunk origins
* WORKING-STORAGE
D0000    DC    HL2'0'              H-S4 PIC S9(4)v0 COMP
         DS    XL6                 reserve the rest of a table
D0001    DC    HL2'0'              H-U4 PIC 9(4)v0 COMP
         DS    XL6                 reserve the rest of a table
D0002    DC    FL4'0'              F-S9 PIC S9(9)v0 COMP
         DS    XL4                 reserve the rest of a table
D0003    DC    HL2'0'              H-S2V2 PIC S9(4)v2 COMP
         DS    XL6                 reserve the rest of a table
D0004    DC    PL3'0'              P-S5 PIC S9(5)v0 COMP-3
         DS    XL5                 reserve the rest of a table
D0005    DC    XL3'00000F'         P-U5 PIC 9(5)v0 COMP-3
         DS    XL5                 reserve the rest of a table
D0006    DC    PL3'0'              P-S3V2 PIC S9(5)v2 COMP-3
         DS    XL5                 reserve the rest of a table
D0007    DC    PL3'0'              P-S4 PIC S9(4)v0 COMP-3
         DS    XL5                 reserve the rest of a table
D0008    DC    ZL4'0'              Z-S4 PIC S9(4)v0 DISP
         DS    XL4                 reserve the rest of a table
D0009    DC    PL10'0'             P-S18 PIC S9(18)v0 COMP-3
         DS    XL6                 reserve the rest of a table
D0010    DS    0CL6                TAB (01 group)
D0011    DC    3HL2'0'             T-H PIC S9(4)v0 COMP table
         DS    XL2                 reserve the rest of a table
D0012    DC    CL1'2'              K PIC 9(1)v0 DISP
*---------------------------------------------------------------
