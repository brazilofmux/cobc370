*---------------------------------------------------------------
* Generated by cobc370 from rptnext.cbl
...
* INITIATE NEXT-RPT
         ...
         USING WSC0000,8
         ST    2,D0004             LINE-COUNTER = 0
         ST    2,RSNG000           no saved next group integer
         LA    2,1
         ST    2,D0005             PAGE-COUNTER = 1
         MVI   RBODY000,X'00'
         MVI   RRH000,X'00'
         MVI   RPF000,X'00'
...
* MOVE 1 -> WS-NUM
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0001(3),S0001      numeric literal as zoned digits
T0003    DS    0H
* GENERATE NORMAL
         DROP  8
...
* MOVE 2 -> WS-NUM
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0001(3),S0002      numeric literal as zoned digits
T0006    DS    0H
* GENERATE NORMAL
         DROP  8
         CLI   RFGEN000,X'00'      the first GENERATE?
...
* MOVE 3 -> WS-NUM
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0001(3),S0003      numeric literal as zoned digits
T0009    DS    0H
* GENERATE FORCED
         DROP  8
         CLI   RFGEN000,X'00'      the first GENERATE?
...
* MOVE 4 -> WS-NUM
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0001(3),S0004      numeric literal as zoned digits
T0012    DS    0H
* GENERATE FORCED
         DROP  8
         CLI   RFGEN000,X'00'      the first GENERATE?
...
* MOVE 5 -> WS-NUM
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0001(3),S0005      numeric literal as zoned digits
T0015    DS    0H
* GENERATE NORMAL
         DROP  8
         CLI   RFGEN000,X'00'      the first GENERATE?
...
RG000    ST    14,RGS000           save the return
         ...
         MVC   RBUF+10(10),S0007   COLUMN literal
         LA    1,RGP000
         L     15,VWRL
         BALR  14,15
         MVI   RRH000,X'01'        the REPORT HEADING is on this page
         MVI   RSUPPR,X'00'
         L     14,RGS000
         BR    14
...
RG001    ST    14,RGS001           save the return
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   RBUF+1(4),S0008     COLUMN literal
         L     8,BL0000            base locator
         USING WSC0000,8
         L     2,D0005
         CVD   2,DWK               binary -> packed
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+6(3),D0010     COLUMN placement
         LA    1,RGP001
         L     15,VWRL
         BALR  14,15
         DROP  8
...
L0025    DS    0H
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   D0012(3),D0001      zoned to zoned, same picture
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+1(3),D0012     COLUMN placement
         MVC   RBUF+8(6),S0009     COLUMN literal
         LA    1,RGP002
         L     15,VWRL
         BALR  14,15
         DROP  8
         LA    2,10                NEXT GROUP NEXT PAGE: FOOTING
         L     8,BL0000            base locator
         USING WSC0000,8
         ST    2,D0004             LINE-COUNTER
...
L0026    DS    0H
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0015(3),D0001      zoned to zoned, same picture
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+1(3),D0015     COLUMN placement
         MVC   RBUF+8(6),S0010     COLUMN literal
         LA    1,RGP003
         L     15,VWRL
         BALR  14,15
         DROP  8
...
RG004    ST    14,RGS004           save the return
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   RBUF+1(12),S0011    COLUMN literal
         L     8,BL0000            base locator
         USING WSC0000,8
         L     2,D0005
         CVD   2,DWK               binary -> packed
         ...
         MVC   EDWK(4),M0001       load the ED pattern
         ED    EDWK(4),EDSRC
         MVC   D0019(3),EDWK+1     the edited result
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+14(3),D0019    COLUMN placement
         LA    1,RGP004
         L     15,VWRL
         BALR  14,15
         DROP  8
...
FC003    DC    F'3'
FC004    DC    F'4'
S0001    DC    CL3'001'            nonnumeric constants
S0002    DC    CL3'002'
S0003    DC    CL3'003'
S0004    DC    CL3'004'
S0005    DC    CL3'005'
S0006    DC    CL17'LINE-COUNTER NOW '
S0007    DC    CL10'TITLE PAGE'
S0008    DC    CL4'PAGE'
S0009    DC    CL6'normal'
S0010    DC    CL6'forced'
S0011    DC    CL12'END, ON PAGE'
...
SPIENUM  DC    H'22'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0H
         ...
         DC    AL2(T0002-COBBEG),AL2(46)
         DC    AL2(T0003-COBBEG),AL2(46)
         DC    AL2(T0004-COBBEG),AL2(46)
         DC    AL2(T0005-COBBEG),AL2(47)
         DC    AL2(T0006-COBBEG),AL2(47)
         DC    AL2(T0007-COBBEG),AL2(47)
         DC    AL2(T0008-COBBEG),AL2(48)
         DC    AL2(T0009-COBBEG),AL2(48)
         DC    AL2(T0010-COBBEG),AL2(48)
         DC    AL2(T0011-COBBEG),AL2(49)
         DC    AL2(T0012-COBBEG),AL2(49)
         DC    AL2(T0013-COBBEG),AL2(49)
         ...
         DC    AL2(T0021-COBBEG),AL2(56)
COBWS    CSECT
WSC0000  EQU   COBWS               chunk origins
* WORKING-STORAGE
         DS    XL4                 reserve the rest of a table
D0000    DC    CL4' '              *RDW PIC X(4)
D0001    DC    CL3'000'            WS-NUM PIC 9(3)v0 DISP
         DS    XL5                 reserve the rest of a table
D0002    DC    CL3' '              WS-LC edited, 3 chars
         DS    XL1                 reserve the rest of a table
D0003    DS    0CL8                NEXT-RPT (01 group)
D0004    DC    FL4'0'              LINE-COUNTER PIC 9(6)v0 COMP
D0005    DC    FL4'0'              PAGE-COUNTER PIC 9(6)v0 COMP
D0006    DS    0C                  RPT-HEAD (02 group)
D0007    DC    CL10' '             RPT0 PIC X(10)
D0008    DS    0C                  PAGE-HEAD (02 group)
D0009    DC    CL4' '              RPT1 PIC X(4)
D0010    DC    CL3' '              RPT2 edited, 3 chars
D0011    DS    0C                  NORMAL (02 group)
D0012    DC    CL3'000'            RPT3 PIC 9(3)v0 DISP
D0013    DC    CL6' '              RPT4 PIC X(6)
D0014    DS    0C                  FORCED (02 group)
D0015    DC    CL3'000'            RPT5 PIC 9(3)v0 DISP
D0016    DC    CL6' '              RPT6 PIC X(6)
D0017    DS    0C                  RPT-FOOT (02 group)
D0018    DC    CL12' '             RPT7 PIC X(12)
D0019    DC    CL3' '              RPT8 edited, 3 chars
*---------------------------------------------------------------
