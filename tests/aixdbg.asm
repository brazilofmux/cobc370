*---------------------------------------------------------------
* Generated by cobc370 from aixdbg.cbl
...
T0000    DS    0H
* OPEN I-O AIX-FILE
         OPEN  (FD000P1,,FD000P2,,FD000)  VSAM paths and the base
         MVI   FD000V,X'01'        paths open
         L     2,FD000X            the key of reference so far
         LA    3,FD000R
         CR    2,3                 the same?
         BE    L0010
         LR    3,2
         SH    3,VSFOUR            its flag
         CLI   0(3),X'00'          ever used?
         BE    L0011
         ENDREQ RPL=(2)            release the old one's hold and posit
         MVI   0(3),X'00'
L0011    DS    0H
L0010    DS    0H
         LA    1,FD000R
         ST    1,FD000X            the key of reference
         CH    15,VSFOUR           a warning?
         BNE   *+6
         SR    15,15               then it opened
         LTR   15,15               VSAM request succeeded?
         BZ    G0001
         SHOWCB ACB=FD000,FIELDS=ERROR,AREA=VSFB,LENGTH=4  DEBUG ACB er
         UNPK  DWK(3),VSFB+3(2)    DEBUG: feedback as hex
         TR    DWK(2),SPIEHEX-240
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0006(2),DWK        DEBUG
         B     G0003
G0001    DS    0H
         DROP  8
         L     8,BL0000            base locator
...
* MOVE K0008     AAAC800 dup dept -> AIX-RECORD
         MVC   D0000(80),S0003     literal move, space padded
T0006    DS    0H
* WRITE AIX-RECORD
         CLI   FD000RA,X'00'       ever used?
         BE    L0014
         ENDREQ RPL=FD000R         no holds before an insert
         MVI   FD000RA,X'00'
L0014    DS    0H
         CLI   FD000Q1A,X'00'      ever used?
         BE    L0015
         ENDREQ RPL=FD000Q1
         MVI   FD000Q1A,X'00'
L0015    DS    0H
         CLI   FD000Q2A,X'00'      ever used?
         BE    L0016
         ENDREQ RPL=FD000Q2
         MVI   FD000Q2A,X'00'
L0016    DS    0H
         PUT   RPL=FD000N          VSAM sequential store
...
* MOVE K0007     DDDC300 dup code -> AIX-RECORD
         MVC   D0000(80),S0007     literal move, space padded
T0013    DS    0H
* WRITE AIX-RECORD
         CLI   FD000RA,X'00'       ever used?
         BE    L0021
...
* MOVE AAA -> AIX-DEPT
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0003(3),S0012      literal move, space padded
T0022    DS    0H
* READ AIX-FILE
         L     2,FD000X            the key of reference so far
         LA    3,FD000Q1
...
L0030    DS    0H
         LA    1,FD000Q1
         ST    1,FD000X            the key of reference
         MODCB RPL=FD000Q1,OPTCD=(DIR,NSP)  by key
         LTR   15,15
         BZ    G0033
...
G0033    DS    0H
         ...
         CLI   FD000Q1A,X'00'      ever used?
         BE    L0032
         ENDREQ RPL=FD000Q1        a direct request reads afresh
         MVI   FD000Q1A,X'00'
L0032    DS    0H
         GET   RPL=FD000Q1         VSAM retrieval by key
         MVI   FD000Q1A,X'01'      the RPL has carried a request
         LTR   15,15               got a record?
         BNZ   L0005
         LTR   15,15               VSAM request succeeded?
         BZ    G0035
         SHOWCB RPL=FD000Q1,FIELDS=FDBK,AREA=VSFB,LENGTH=4  why did it
...
G0035    DS    0H
         ...
         USING WSC0000,8
         MVC   D0006(2),=C'00'
         SHOWCB RPL=FD000Q1,FIELDS=FDBK,AREA=VSFB,LENGTH=4  a duplicate
         CLI   VSFB+3,X'08'
         BNE   G0039
         MVC   D0006(2),=C'02'     successful, duplicate alternate key
G0039    DS    0H
...
G0038    DS    0H
         CLI   FD000Q1A,X'00'      ever used?
         BE    L0033
         ENDREQ RPL=FD000Q1        the failed request is over
         MVI   FD000Q1A,X'00'
L0033    DS    0H
G0036    DS    0H
         B     L0006
...
* READ AIX-FILE
         ...
         LA    2,FD000R
         CR    1,2                 the base string?
         BNE   L0039
         MODCB RPL=(1),OPTCD=(SEQ,UPD)  next, for update
         B     L0040
L0039    DS    0H
         MODCB RPL=(1),OPTCD=(SEQ)  next, through a path
L0040    DS    0H
         LTR   15,15
         BZ    G0045
         L     8,BL0000            base locator
...
G0045    DS    0H
         DROP  8
         L     1,FD000X            the key of reference's RPL
         GET   RPL=(1)             VSAM sequential retrieval
         L     1,FD000X
         SH    1,VSFOUR
         MVI   0(1),X'01'          the RPL has carried a request
         LTR   15,15               got a record?
         BNZ   L0007
         LTR   15,15               VSAM request succeeded?
         BZ    G0047
         L     1,FD000X            the key of reference's RPL
         SHOWCB RPL=(1),FIELDS=FDBK,AREA=VSFB,LENGTH=4  why did it fail
...
G0050    DS    0H
         ...
         CLI   0(3),X'00'          ever used?
         BE    L0041
         ENDREQ RPL=(2)            the failed request is over
         MVI   0(3),X'00'
L0041    DS    0H
G0048    DS    0H
         B     L0008
...
* IF
         L     8,BL0000            base locator
         USING WSC0000,8
         CLC   D0006(2),S0015      alphanumeric compare
         BE    L0009
T0029    DS    0H
* PERFORM SHOW
...
* DISPLAY
         ...
         MVC   DSPBUF+2(1),S0016
         MVC   DSPBUF+3(5),D0001+0
         MVC   DSPBUF+8(1),S0016
         MVC   DSPBUF+9(3),D0003+0
         MVC   DSPBUF+12(1),S0016
...
PT030    DC    H'0'                PERFORM n TIMES counter
X0001    DC    A(F0001)            PATH-READ
X0002    DC    A(F0002)            NEXT-ONE
X0003    DC    A(F0003)            SHOW
VDISP    DC    V(COBDISP)
VTERM    DC    V(COBTERM)
...
WK5      DS    PL16
* file control blocks
FD000    ACB   DDNAME=AIXBAS,MACRF=(KEY,SEQ,DIR,IN)  VSAM access method
...
FD000N   RPL   ACB=FD000,AREA=D0000,                                   X
               AREALEN=80,RECLEN=80,ARG=D0001,KEYLEN=5,OPTCD=(KEY,SEQ, X
               KEQ,NUP,MVE)
FD000P1  ACB   DDNAME=AIXBA01,MACRF=(KEY,SEQ,DIR,OUT)  AIX-DEPT
...
FD000Q1   RPL   ACB=FD000P1,AREA=D0000,                                X
               ...
               KEQ,NUP,MVE)
FD000P2  ACB   DDNAME=AIXBA02,MACRF=(KEY,SEQ,DIR,OUT)  AIX-CODE
...
S0001    DC    CL9'OPEN I-O '      nonnumeric constants
S0002    DC    CL11'STEP 1 DONE'
S0003    DC    CL48'K0008     AAAC800 dup dept                      '
         DC    CL32'                                '
S0004    DC    CL20'WRITE K0008 REFUSED '
S0005    DC    CL12'WRITE K0008 '
S0006    DC    CL11'STEP 2 DONE'
S0007    DC    CL48'K0007     DDDC300 dup code                      '
         DC    CL32'                                '
S0008    DC    CL20'WRITE K0007 REFUSED '
S0009    DC    CL12'WRITE K0007 '
S0010    DC    CL11'STEP 3 DONE'
S0011    DC    CL6'CLOSE '
S0012    DC    CL3'AAA'
S0013    DC    CL6'NO AAA'
S0014    DC    CL7'AT END '
...
SPIENUM  DC    H'31'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0H
         ...
         DC    AL2(T0002-COBBEG),AL2(35)
         DC    AL2(T0003-COBBEG),AL2(36)
         ...
         DC    AL2(T0026-COBBEG),AL2(62)
         DC    AL2(T0027-COBBEG),AL2(62)
         DC    AL2(T0028-COBBEG),AL2(63)
         DC    AL2(T0029-COBBEG),AL2(63)
         DC    AL2(T0030-COBBEG),AL2(65)
COBWS    CSECT
