*---------------------------------------------------------------
* Generated by cobc370 from rptpage.cbl
...
* MOVE ALPHA  -> WS-NAME
         ...
         MVC   0(6,6),S0001        literal move, space padded
T0001    DS    0H
* MOVE BETA   -> WS-NAME
         LA    6,1                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0005(6)          element address
         MVC   0(6,6),S0002        literal move, space padded
T0002    DS    0H
* MOVE GAMMA  -> WS-NAME
         LA    6,2                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0005(6)          element address
         MVC   0(6,6),S0003        literal move, space padded
T0003    DS    0H
...
* INITIATE PAGED-RPT
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         ST    2,D0008             LINE-COUNTER = 0
         ST    2,RSNG000           no saved next group integer
         LA    2,1
         ST    2,D0009             PAGE-COUNTER = 1
         MVI   RBODY000,X'00'
         MVI   RRH000,X'00'
         MVI   RPF000,X'00'
...
* MOVE LINE-COUNTER -> WS-LC
         ...
         ED    EDWK(4),EDSRC
         MVC   D0003(3),EDWK+1     the edited result
T0007    DS    0H
* DISPLAY
         MVC   DSPBUF+0(24),S0004
         MVC   DSPBUF+24(3),D0003+0
         LA    1,PARM0001
         L     15,VDISP
         BALR  14,15
T0008    DS    0H
...
T0017    DS    0H
* ADD 1 -> WS-K
         PACK  PWK1(16),D0006(1)   zoned -> packed
...
T0019    DS    0H
* MOVE 1 -> WS-K
         MVC   D0006(1),S0005      numeric literal as zoned digits
         DROP  8
L0001    DS    0H
T0020    DS    0H
* GENERATE TWO-LINER
...
RG000    ST    14,RGS000           save the return
         ...
         MVC   RBUF+1(12),S0006    COLUMN literal
         MVC   RBUF+20(4),S0007    COLUMN literal
         L     8,BL0000            base locator
         USING WSC0000,8
         L     2,D0009
         CVD   2,DWK               binary -> packed
         ...
         MVC   EDWK(4),M0001       load the ED pattern
         ED    EDWK(4),EDSRC
         MVC   D0013(3),EDWK+1     the edited result
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+25(3),D0013    COLUMN placement
         LA    1,RGP000
         L     15,VWRL
         BALR  14,15
         DROP  8
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   RBUF+1(3),S0008     COLUMN literal
         MVC   RBUF+8(4),S0009     COLUMN literal
         LA    1,RGP000
         L     15,VWRL
         BALR  14,15
         DROP  8
...
L0020    DS    0H
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   D0017(3),D0002      zoned to zoned, same picture
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+1(3),D0017     COLUMN placement
         PACK  DWK(8),D0006(1)     subscript
         CVB   7,DWK
         BCTR  7,0                 subscript-1
         MH    7,H0001             times element size
         LA    7,D0005(7)          element address
         MVC   D0018(6),0(7)       alphanumeric move
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+8(6),D0018     COLUMN placement
         LA    1,RGP001
         L     15,VWRL
         BALR  14,15
         DROP  8
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   RBUF+8(9),S0010     COLUMN literal
         LA    1,RGP001
         L     15,VWRL
         BALR  14,15
         DROP  8
...
L0024    DS    0H
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   D0021(3),D0002      zoned to zoned, same picture
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   RBUF+1(3),D0021     COLUMN placement
...
L0025    DS    0H
         ...
         MVI   RBUF+1,C' '
         MVC   RBUF+2(133),RBUF+1  blank the line
         MVC   RBUF+1(13),S0012    COLUMN literal
         LA    1,RGP003
         L     15,VWRL
         BALR  14,15
         MVI   RBODY000,X'01'      a body group is on this page
...
X0001    DC    A(F0001)            TWO-PARA
X0002    DC    A(F0002)            ONE-PARA
VDISP    DC    V(COBDISP)
VTERM    DC    V(COBTERM)
...
S0002    DC    CL6'BETA  '
S0003    DC    CL6'GAMMA '
S0004    DC    CL24'LINE-COUNTER AFTER FIVE '
S0005    DC    CL1'1'
S0006    DC    CL12'PAGED REPORT'
S0007    DC    CL4'PAGE'
S0008    DC    CL3'NUM'
S0009    DC    CL4'NAME'
S0010    DC    CL9'(second) '
S0011    DC    CL6'single'
S0012    DC    CL13'-- LINE 10 --'
...
SPIENUM  DC    H'23'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0H
         DC    AL2(T0000-COBBEG),AL2(54)
         DC    AL2(T0001-COBBEG),AL2(55)
         DC    AL2(T0002-COBBEG),AL2(56)
         DC    AL2(T0003-COBBEG),AL2(57)
         DC    AL2(T0004-COBBEG),AL2(58)
         DC    AL2(T0005-COBBEG),AL2(59)
         DC    AL2(T0006-COBBEG),AL2(61)
         DC    AL2(T0007-COBBEG),AL2(62)
         DC    AL2(T0008-COBBEG),AL2(63)
         DC    AL2(T0009-COBBEG),AL2(65)
         DC    AL2(T0010-COBBEG),AL2(66)
         DC    AL2(T0011-COBBEG),AL2(68)
         DC    AL2(T0012-COBBEG),AL2(69)
         DC    AL2(T0013-COBBEG),AL2(71)
         DC    AL2(T0014-COBBEG),AL2(72)
         DC    AL2(T0015-COBBEG),AL2(73)
         DC    AL2(T0016-COBBEG),AL2(75)
         DC    AL2(T0017-COBBEG),AL2(76)
         DC    AL2(T0018-COBBEG),AL2(77)
         DC    AL2(T0019-COBBEG),AL2(77)
         DC    AL2(T0020-COBBEG),AL2(78)
         DC    AL2(T0021-COBBEG),AL2(80)
         DC    AL2(T0022-COBBEG),AL2(81)
COBWS    CSECT
...
D0002    DC    CL3'000'            WS-NUM PIC 9(3)v0 DISP
         DS    XL5                 reserve the rest of a table
D0003    DC    CL3' '              WS-LC edited, 3 chars
         DS    XL5                 reserve the rest of a table
D0004    DS    0CL18               WS-TAB (01 group)
D0005    DC    3CL6' '             WS-NAME PIC X(6) table
         DS    XL6                 reserve the rest of a table
D0006    DC    CL1'0'              WS-K PIC 9(1)v0 DISP
         DS    XL3                 reserve the rest of a table
D0007    DS    0CL8                PAGED-RPT (01 group)
D0008    DC    FL4'0'              LINE-COUNTER PIC 9(6)v0 COMP
D0009    DC    FL4'0'              PAGE-COUNTER PIC 9(6)v0 COMP
D0010    DS    0C                  PAGE-HEAD (02 group)
D0011    DC    CL12' '             RPT0 PIC X(12)
D0012    DC    CL4' '              RPT1 PIC X(4)
D0013    DC    CL3' '              RPT2 edited, 3 chars
D0014    DC    CL3' '              RPT3 PIC X(3)
D0015    DC    CL4' '              RPT4 PIC X(4)
D0016    DS    0C                  TWO-LINER (02 group)
D0017    DC    CL3'000'            RPT5 PIC 9(3)v0 DISP
D0018    DC    CL6' '              RPT6 PIC X(6)
D0019    DC    CL9' '              RPT7 PIC X(9)
D0020    DS    0C                  ONE-LINER (02 group)
D0021    DC    CL3'000'            RPT8 PIC 9(3)v0 DISP
D0022    DC    CL6' '              RPT9 PIC X(6)
D0023    DS    0C                  FIXED-LINER (02 group)
D0024    DC    CL13' '             RPT10 PIC X(13)
*---------------------------------------------------------------
