*---------------------------------------------------------------
* Generated by cobc370 from vrec.cbl
...
T0003    DS    0H
* WRITE VO-LONG
         MVC   D0000(2),H0001      RDW: the length of this record
         XC    D0000+2(2),D0000+2
         PUT   FD000,D0000         variable-length record
         DROP  8
L0002    DS    0H
T0004    DS    0H
* MOVE S002 -> VOS-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(4),S0003      literal move, space padded
T0005    DS    0H
* MOVE short2 -> VOS-TEXT
         MVC   D0003(6),S0004      literal move, space padded
T0006    DS    0H
* WRITE VO-SHORT
         MVC   D0000(2),H0002      RDW: the length of this record
         XC    D0000+2(2),D0000+2
         PUT   FD000,D0000         variable-length record
         DROP  8
L0004    DS    0H
...
* MOVE L003 -> VOL-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0005(4),S0005      literal move, space padded
T0008    DS    0H
* MOVE third long record, also thirty-six. -> VOL-TEXT
         MVC   D0006(36),S0006     literal move, space padded
T0009    DS    0H
* WRITE VO-LONG
         MVC   D0000(2),H0001      RDW: the length of this record
         XC    D0000+2(2),D0000+2
...
* MOVE S004 -> VOS-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(4),S0007      literal move, space padded
T0011    DS    0H
* MOVE short4 -> VOS-TEXT
         MVC   D0003(6),S0008      literal move, space padded
T0012    DS    0H
* WRITE VO-SHORT
         MVC   D0000(2),H0002      RDW: the length of this record
         XC    D0000+2(2),D0000+2
...
* MOVE S005 -> VOS-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(4),S0009      literal move, space padded
T0014    DS    0H
* MOVE short5 -> VOS-TEXT
         MVC   D0003(6),S0010      literal move, space padded
T0015    DS    0H
* WRITE VO-SHORT
         MVC   D0000(2),H0002      RDW: the length of this record
         XC    D0000+2(2),D0000+2
...
* READ VUPD
         ...
         LA    2,D0007             the RDW cell
         LH    3,0(0,1)            the record's length, RDW included
         LR    4,1
         LR    5,3
         MVCL  2,4                 into the record area
         B     L0012
L0011    DS    0H                  AT END
         DROP  8
T0021    DS    0H
* MOVE Y -> WS-EOF
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0021(1),S0012      literal move, space padded
         DROP  8
L0012    DS    0H
T0022    DS    0H
* READ VUPD
...
L0014    DS    0H
T0024    DS    0H
* MOVE SHORT2 -> VUS-TEXT
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0010(6),S0013      literal move, space padded
T0025    DS    0H
* REWRITE VU-SHORT
         L     1,U001              where the last READ left the record
         LR    2,1
         LH    3,0(0,1)            the length in the buffer's RDW
         LA    4,D0007
         LR    5,3
         MVCL  2,4                 back into the buffer
...
* MOVE N -> WS-EOF
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0021(1),S0016      literal move, space padded
T0036    DS    0H
* MOVE SPACES -> VI-LONG
         LA    1,D0018             SPACES
         MVI   0(1),C' '
...
* READ VIN
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         GET   FD002,D0014         QSAM move mode: RDW, then the record
         B     L0022
L0021    DS    0H                  AT END
         DROP  8
T0040    DS    0H
...
T0042    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),D0019+0
         MVC   DSPBUF+4(2),S0017
         MVC   DSPBUF+6(36),D0020+0
         MVC   DSPBUF+42(1),S0018
         LA    1,PARM0003
         L     15,VDISP
         BALR  14,15
         DROP  8
...
WK5      DS    PL16
* file control blocks
FD000    DCB   DDNAME=VRECF,DSORG=PS,MACRF=(PM),RECFM=VB,              X
               LRECL=44,BLKSIZE=136
FD001    DCB   DDNAME=VRECF,DSORG=PS,MACRF=(GL,PL)
FD002    DCB   DDNAME=VRECF,DSORG=PS,MACRF=(GM)
U001     DS    F                   the record the last GET located
H0001    DC    H'44'               element sizes
H0002    DC    H'14'
S0001    DC    CL4'L001'           nonnumeric constants
S0002    DC    CL36'first long record, thirty-six chars.'
S0003    DC    CL4'S002'
S0004    DC    CL6'short2'
S0005    DC    CL4'L003'
S0006    DC    CL36'third long record, also thirty-six. '
S0007    DC    CL4'S004'
S0008    DC    CL6'short4'
S0009    DC    CL4'S005'
S0010    DC    CL6'short5'
...
SPIENUM  DC    H'43'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0H
         DC    AL2(T0000-COBBEG),AL2(49)
         DC    AL2(T0001-COBBEG),AL2(50)
         DC    AL2(T0002-COBBEG),AL2(51)
         DC    AL2(T0003-COBBEG),AL2(52)
         DC    AL2(T0004-COBBEG),AL2(53)
         DC    AL2(T0005-COBBEG),AL2(53)
         DC    AL2(T0006-COBBEG),AL2(54)
         DC    AL2(T0007-COBBEG),AL2(55)
         DC    AL2(T0008-COBBEG),AL2(56)
         DC    AL2(T0009-COBBEG),AL2(57)
         DC    AL2(T0010-COBBEG),AL2(58)
         DC    AL2(T0011-COBBEG),AL2(58)
         DC    AL2(T0012-COBBEG),AL2(59)
         DC    AL2(T0013-COBBEG),AL2(60)
         DC    AL2(T0014-COBBEG),AL2(60)
         DC    AL2(T0015-COBBEG),AL2(61)
         DC    AL2(T0016-COBBEG),AL2(62)
         DC    AL2(T0017-COBBEG),AL2(63)
         DC    AL2(T0018-COBBEG),AL2(64)
         DC    AL2(T0019-COBBEG),AL2(65)
         DC    AL2(T0020-COBBEG),AL2(66)
         DC    AL2(T0021-COBBEG),AL2(66)
         DC    AL2(T0022-COBBEG),AL2(67)
         DC    AL2(T0023-COBBEG),AL2(67)
         DC    AL2(T0024-COBBEG),AL2(68)
         DC    AL2(T0025-COBBEG),AL2(69)
         DC    AL2(T0026-COBBEG),AL2(70)
         DC    AL2(T0027-COBBEG),AL2(70)
         DC    AL2(T0028-COBBEG),AL2(71)
         DC    AL2(T0029-COBBEG),AL2(72)
         DC    AL2(T0030-COBBEG),AL2(73)
         DC    AL2(T0031-COBBEG),AL2(74)
         DC    AL2(T0032-COBBEG),AL2(75)
         DC    AL2(T0033-COBBEG),AL2(76)
         DC    AL2(T0034-COBBEG),AL2(78)
         DC    AL2(T0035-COBBEG),AL2(79)
         DC    AL2(T0036-COBBEG),AL2(80)
         DC    AL2(T0037-COBBEG),AL2(81)
         DC    AL2(T0038-COBBEG),AL2(82)
         DC    AL2(T0039-COBBEG),AL2(84)
         DC    AL2(T0040-COBBEG),AL2(84)
         DC    AL2(T0041-COBBEG),AL2(86)
         DC    AL2(T0042-COBBEG),AL2(86)
COBWS    CSECT
WSC0000  EQU   COBWS               chunk origins
* WORKING-STORAGE
         DS    XL4                 reserve the rest of a table
D0000    DC    CL4' '              *RDW PIC X(4)
D0001    DS    0CL10               VO-SHORT (01 group)
D0002    DC    CL4' '              VOS-KEY PIC X(4)
D0003    DC    CL6' '              VOS-TEXT PIC X(6)
D0004    EQU   COBWS+8             VO-LONG REDEFINES
D0005    EQU   COBWS+8             VOL-KEY REDEFINES
D0006    EQU   COBWS+12            VOL-TEXT REDEFINES
         DS    XL34                reserve the rest of a table
D0007    DC    CL4' '              *RDW PIC X(4)
D0008    DS    0CL10               VU-SHORT (01 group)
D0009    DC    CL4' '              VUS-KEY PIC X(4)
D0010    DC    CL6' '              VUS-TEXT PIC X(6)
D0011    EQU   COBWS+56            VU-LONG REDEFINES
D0012    EQU   COBWS+56            VUL-KEY REDEFINES
D0013    EQU   COBWS+60            VUL-TEXT REDEFINES
         DS    XL34                reserve the rest of a table
D0014    DC    CL4' '              *RDW PIC X(4)
D0015    DS    0CL10               VI-SHORT (01 group)
D0016    DC    CL4' '              VIS-KEY PIC X(4)
D0017    DC    CL6' '              VIS-TEXT PIC X(6)
D0018    EQU   COBWS+104           VI-LONG REDEFINES
D0019    EQU   COBWS+104           VIL-KEY REDEFINES
D0020    EQU   COBWS+108           VIL-TEXT REDEFINES
         DS    XL30                reserve the rest of a table
D0021    DC    CL1'N'              WS-EOF PIC X(1)
*---------------------------------------------------------------
