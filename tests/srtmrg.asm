*---------------------------------------------------------------
* Generated by cobc370 from srtmrg.cbl
...
T0000    DS    0H
* SORT SORT-FILE
         MVC   MRGSV2(4),MRGNXT    no MERGE while the sort runs
         XC    MRGNXT(4),MRGNXT
         ...
         USING WSC0000,8
         L     15,D0012            SORT-FILE-SIZE
         LTR   15,15
         BNP   L0031
         CVD   15,DWK
         UNPK  SRT0001Z(7),DWK+4(4)
         OI    SRT0001Z+6,X'F0'
         LA    15,SRT0001G
         B     L0032
L0031    DS    0H
         LA    15,SRT0001F
L0032    DS    0H
         ST    15,SRT0001E+4       the SORT statement's last byte
         DROP  8
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         STH   15,D0011            SORT-RETURN
         B     L0003               past the input and output parts
         DROP  8
L0001    DS    0H
T0001    DS    0H
* OPEN INPUT IN-FILE
...
* MOVE IN-REC -> S-REC
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0007(80),D0001     alphanumeric move
T0005    DS    0H
* RELEASE: the record to the sort, and back here for the next
         LA    14,L0035
         ST    14,SRTRES
         ST    12,SRTR12
         LA    1,D0007             the record
...
* OPEN OUTPUT W1-FILE
         ...
         BO    L0036
         WTO   'COBC370: OPEN FAILED, DD W1FILE',ROUTCDE=11
         ABEND 35
         B     L0037
L0036    DS    0H
L0037    DS    0H
...
* RETURN SORT-FILE
         ...
         BZ    L0010
         B     L0041               it is in the SD's record
L0040    DS    0H
...
L0039    DS    0H
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0007+0(80),0(1)    into the SD's record
L0041    DS    0H
         DROP  8
         B     L0011
L0010    DS    0H                  AT END
...
* MOVE S-REC -> W1-REC
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0005(80),D0007     alphanumeric move
T0013    DS    0H
* WRITE W1-REC
         PUT   FD002,D0005
T0014    DS    0H
...
* OPEN INPUT IN2-FILE
         ...
         BO    L0044
         WTO   'COBC370: OPEN FAILED, DD INFILE2',ROUTCDE=11
         ABEND 35
         B     L0045
L0044    DS    0H
L0045    DS    0H
T0021    DS    0H
         LA    14,L0016
         ST    14,MRGRET
         B     L0014               the file's first record
L0016    DS    0H
T0022    DS    0H
         LA    14,L0017
         ST    14,MRGRET
         B     L0015               the file's first record
L0017    DS    0H
T0023    DS    0H
* PERFORM SHOW-IT
...
T0035    DS    0H
         ...
         BR    14                  back to whoever asked for the READ
T0036    DS    0H
* MERGE SORT-FILE: the next record
L0013    DS    0H
         ST    14,MRGRT
         SR    4,4                 no record yet
         CLI   MRGF000+0,0         W1-FILE
         BNE   L0053               at end
         L     5,MRGA000+0         its record
         LTR   4,4
         BZ    L0054               the first candidate
         BAL   14,L0050            compare
         LTR   15,15
         BZ    L0053               the best so far stays
L0054    DS    0H
         LR    4,5
         LA    5,0
         ST    5,MRGBST            the file it came from
L0053    DS    0H
         CLI   MRGF000+1,0         IN2-FILE
         BNE   L0055               at end
...
L0055    DS    0H
         LTR   4,4
         BZ    L0048               every file at end
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0007+0(80),0(4)    into the SD's record
         DROP  8
         L     15,MRGBST
         SLL   15,2
...
L0048    DS    0H
         ...
         L     14,MRGRT
         BR    14
L0050    DS    0H                  candidate before the best?
         CLC   0(5,4),0(5)         S-KEY
         BH    L0051               ASCENDING
         BL    L0052
L0052    DS    0H                  equal: the first-named file's
         SR    15,15
         BR    14
L0051    DS    0H
         LA    15,1
...
L0057    DS    0H
         LA    15,SRT0053F
L0058    DS    0H
         ST    15,SRT0053E+4       the SORT statement's last byte
         DROP  8
         LA    1,SRTSAVE           a save area of its own, so the
         ST    13,4(,1)            procedures can use SAVEAREA while
         LR    13,1                the sort is running
         ...
         USING WSC0000,8
         STH   15,D0011            SORT-RETURN
         B     L0024               past the input and output parts
         DROP  8
L0022    DS    0H
T0039    DS    0H
* OPEN INPUT IN-FILE
...
* RETURN SORT-FILE
         ...
         LTR   1,1                 zero: no more
         BZ    L0029
         B     L0067               it is in the SD's record
L0066    DS    0H
         CLI   SRTHAVE,1           a record the sort handed over?
         BE    L0065
         LA    14,L0064
...
T0054    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0004
         MVC   DSPBUF+4(20),D0010+0
         LA    1,PARM0004
         L     15,VDISP
         BALR  14,15
T0055    DS    0H
...
SRT0001S DC    C' SORT FIELDS=(0001,005,CH,A)'  SORT statement
SRT0001F EQU   *-1                 its end without SIZE
         DC    C',SIZE=E'
SRT0001Z DC    C'0000000'          SORT-FILE-SIZE, when positive
SRT0001G EQU   *-1                 its end with SIZE
SRT0001R DC    C' RECORD TYPE=F,LENGTH=(00080)'  RECORD statement
SRT0001Q EQU   *-1
         DS    0F
         DC    H'0'
SRT0001L DC    AL2(24)             sort parameter list: its length
SRT0001E DC    A(SRT0001S,SRT0001F,SRT0001R,SRT0001Q)  SORT and RECORD,
         DC    A(SRTE15,SRTE35)    the exits
SRT0001P DC    X'80',AL3(SRT0001L)  R1 -> this
MRGF000  DS    XL16                MERGE: which USING files are at end
MRGA000  DS    16F                 each file's record area
MRGT000  DC    A(L0014,L0015)      each file's READ
SRT0053S DC    C' SORT FIELDS=(0001,005,CH,D)'  SORT statement
SRT0053F EQU   *-1                 its end without SIZE
         DC    C',SIZE=E'
SRT0053Z DC    C'0000000'          SORT-FILE-SIZE, when positive
SRT0053G EQU   *-1                 its end with SIZE
SRT0053R DC    C' RECORD TYPE=F,LENGTH=(00080)'  RECORD statement
SRT0053Q EQU   *-1
         DS    0F
         DC    H'0'
SRT0053L DC    AL2(24)             sort parameter list: its length
SRT0053E DC    A(SRT0053S,SRT0053F,SRT0053R,SRT0053Q)  SORT and RECORD,
         DC    A(SRTE15,SRTE35)    the exits
SRT0053P DC    X'80',AL3(SRT0053L)  R1 -> this
PWK1     DS    PL16
PWK2     DS    PL16
...
FD001    DCB   DDNAME=INFILE2,DSORG=PS,MACRF=(GM)
FD002    DCB   DDNAME=W1FILE,DSORG=PS,MACRF=(GM,PM),RECFM=FB,          X
               LRECL=80,BLKSIZE=80
S0001    DC    CL5'MERGE'          nonnumeric constants
S0002    DC    CL15'SORT DESCENDING'
S0003    DC    CL3'END'
S0004    DC    CL4'RET '
...
PA0001   DC    A(P0001)            SHOW-IT
PA0003   DC    A(P0003)            S-EXIT
...
SPIELTB  DS    0F
         DC    A(T0000-COBBEG),AL2(32,0)
         DC    A(T0001-COBBEG),AL2(33,0)
         DC    A(T0002-COBBEG),AL2(33,0)
         DC    A(T0003-COBBEG),AL2(33,0)
         DC    A(T0004-COBBEG),AL2(33,0)
         DC    A(T0005-COBBEG),AL2(33,0)
         DC    A(T0006-COBBEG),AL2(33,0)
         DC    A(T0007-COBBEG),AL2(33,0)
         DC    A(T0008-COBBEG),AL2(33,0)
         DC    A(T0009-COBBEG),AL2(33,0)
         DC    A(T0010-COBBEG),AL2(33,0)
         DC    A(T0011-COBBEG),AL2(33,0)
         DC    A(T0012-COBBEG),AL2(33,0)
         DC    A(T0013-COBBEG),AL2(33,0)
         DC    A(T0014-COBBEG),AL2(33,0)
         DC    A(T0015-COBBEG),AL2(33,0)
         DC    A(T0016-COBBEG),AL2(33,0)
         DC    A(T0017-COBBEG),AL2(34,0)
         DC    A(T0018-COBBEG),AL2(35,0)
         DC    A(T0019-COBBEG),AL2(36,0)
         DC    A(T0020-COBBEG),AL2(36,0)
         DC    A(T0021-COBBEG),AL2(36,0)
         DC    A(T0022-COBBEG),AL2(36,0)
         DC    A(T0023-COBBEG),AL2(36,0)
         DC    A(T0024-COBBEG),AL2(36,0)
         DC    A(T0025-COBBEG),AL2(36,0)
         DC    A(T0026-COBBEG),AL2(36,0)
         DC    A(T0027-COBBEG),AL2(36,0)
         DC    A(T0028-COBBEG),AL2(36,0)
         DC    A(T0029-COBBEG),AL2(36,0)
         DC    A(T0030-COBBEG),AL2(36,0)
         DC    A(T0031-COBBEG),AL2(36,0)
         DC    A(T0032-COBBEG),AL2(36,0)
         DC    A(T0033-COBBEG),AL2(36,0)
         ...
         DC    A(T0039-COBBEG),AL2(39,0)
         DC    A(T0040-COBBEG),AL2(39,0)
         DC    A(T0041-COBBEG),AL2(39,0)
         DC    A(T0042-COBBEG),AL2(39,0)
         DC    A(T0043-COBBEG),AL2(39,0)
         DC    A(T0044-COBBEG),AL2(39,0)
         DC    A(T0045-COBBEG),AL2(39,0)
         DC    A(T0046-COBBEG),AL2(39,0)
         DC    A(T0047-COBBEG),AL2(39,0)
         DC    A(T0048-COBBEG),AL2(39,0)
         DC    A(T0049-COBBEG),AL2(40,0)
         DC    A(T0050-COBBEG),AL2(41,0)
         DC    A(T0051-COBBEG),AL2(44,0)
         DC    A(T0052-COBBEG),AL2(44,0)
         DC    A(T0053-COBBEG),AL2(45,0)
         DC    A(T0054-COBBEG),AL2(46,0)
         DC    A(T0055-COBBEG),AL2(47,0)
         DC    A(T0056-COBBEG),AL2(49,0)
...
D0003    DC    CL80' '             IN2-REC PIC X(80)
         DS    XL4                 reserve the rest of a table
D0004    DC    CL4' '              *RDW PIC X(4)
D0005    DC    CL80' '             W1-REC PIC X(80)
         DS    XL4                 reserve the rest of a table
D0006    DC    CL4' '              *RDW PIC X(4)
D0007    DS    0CL80               S-REC (01 group)
D0008    DC    CL5' '              S-KEY PIC X(5)
D0009    DC    CL75' '             S-REST PIC X(75)
D0010    DC    CL20' '             SHOWN PIC X(20)
D0011    DC    HL2'0'              SORT-RETURN PIC S9(4)v0 COMP
         DS    XL2                 reserve the rest of a table
D0012    DC    FL4'0'              SORT-FILE-SIZE PIC S9(8)v0 COMP
*---------------------------------------------------------------
