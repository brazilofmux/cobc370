*---------------------------------------------------------------
* Generated by cobc370 from subs2d.cbl
* Standard OS/360 entry linkage. SYS1.COBLIB is never referenced.
* DISPLAY and report output are served by our own COBRT runtime.
*---------------------------------------------------------------
SUBS2D   CSECT
         STM   14,12,12(13)        save caller's registers
         BALR  12,0                first code base
COBBEG   EQU   *
         USING COBBEG,12
         LA    11,2048(,12)        second code base
         LA    11,2048(,11)
         USING COBBEG+4096,11
         LA    10,2048(,11)        third code base
         LA    10,2048(,10)
         USING COBBEG+8192,10
         ST    13,SAVEAREA+4       backward chain to caller
         LA    0,SAVEAREA
         ST    0,8(13)             forward chain from caller
         LR    13,0                our save area is now current
         SR    0,0
         SPM   0                   no overflow interrupts: high-order t
         SPIE  COBSPIE,((1,7),9,(11,12),15)  report program checks by l
T0000    DS    0H
* MOVE A1 -> CELL
         LA    6,0                 subscript-1
         MH    6,H0001             times element size
         LA    0,0                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         L     8,BL0000            base locator
         USING WSC0000,8
         LA    6,D0002(6)          element address
         MVC   0(2,6),S0001        literal move, space padded
T0001    DS    0H
* MOVE A4 -> CELL
         LA    6,0                 subscript-1
         MH    6,H0001             times element size
         LA    0,3                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    6,D0002(6)          element address
         MVC   0(2,6),S0002        literal move, space padded
T0002    DS    0H
* MOVE B2 -> CELL
         LA    6,1                 subscript-1
         MH    6,H0001             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    6,D0002(6)          element address
         MVC   0(2,6),S0003        literal move, space padded
T0003    DS    0H
* MOVE C3 -> CELL
         LA    6,2                 subscript-1
         MH    6,H0001             times element size
         LA    0,2                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    6,D0002(6)          element address
         MVC   0(2,6),S0004        literal move, space padded
T0004    DS    0H
* MOVE C4 -> CELL
         LA    6,2                 subscript-1
         MH    6,H0001             times element size
         LA    0,3                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    6,D0002(6)          element address
         MVC   0(2,6),S0005        literal move, space padded
T0005    DS    0H
* MOVE CELL -> SEEN
         LA    7,0                 subscript-1
         MH    7,H0001             times element size
         LA    0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0006    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0006
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0001
         L     15,VDISP
         BALR  14,15
T0007    DS    0H
* MOVE CELL -> SEEN
         LA    7,0                 subscript-1
         MH    7,H0001             times element size
         LA    0,3                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0008    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0007
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0002
         L     15,VDISP
         BALR  14,15
T0009    DS    0H
* MOVE CELL -> SEEN
         LA    7,1                 subscript-1
         MH    7,H0001             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0010    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0008
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0003
         L     15,VDISP
         BALR  14,15
T0011    DS    0H
* MOVE CELL -> SEEN
         LA    7,2                 subscript-1
         MH    7,H0001             times element size
         LA    0,2                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0012    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0009
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0004
         L     15,VDISP
         BALR  14,15
T0013    DS    0H
* MOVE CELL -> SEEN
         LA    7,2                 subscript-1
         MH    7,H0001             times element size
         LA    0,3                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0014    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0010
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0005
         L     15,VDISP
         BALR  14,15
T0015    DS    0H
* MOVE 2 -> I
         ZAP   PWK1(16),K0001(16)  literal
         UNPK  D0007(1),PWK1(16)   packed -> zoned
         OI    D0007+0,X'F0'       unsigned: force an F zone
T0016    DS    0H
* MOVE 2 -> J
         ZAP   PWK1(16),K0001(16)  literal
         UNPK  D0008(1),PWK1(16)   packed -> zoned
         OI    D0008+0,X'F0'       unsigned: force an F zone
T0017    DS    0H
* MOVE CELL -> SEEN
         PACK  DWK(8),D0007(1)     subscript
         CVB   7,DWK
         BCTR  7,0                 subscript-1
         MH    7,H0001             times element size
         PACK  DWK(8),D0008(1)     subscript
         CVB   0,DWK
         BCTR  0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0018    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0011
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0006
         L     15,VDISP
         BALR  14,15
T0019    DS    0H
* MOVE 3 -> R
         ZAP   PWK1(16),K0002(16)  literal
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK               packed -> binary
         STH   2,D0013
T0020    DS    0H
* MOVE 4 -> C
         ZAP   PWK1(16),K0003(16)  literal
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK               packed -> binary
         STH   2,D0014
T0021    DS    0H
* MOVE CELL -> SEEN
         LH    7,D0013             subscript
         BCTR  7,0                 subscript-1
         MH    7,H0001             times element size
         LH    0,D0014             subscript
         BCTR  0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0022    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0012
         MVC   DSPBUF+3(2),D0010
         LA    1,PARM0007
         L     15,VDISP
         BALR  14,15
T0023    DS    0H
* MOVE P -> SPOT
         LA    6,0                 subscript-1
         MH    6,H0003             times element size
         LA    0,0                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    0,0                 subscript-1
         AR    6,0                 add this dimension
         LA    6,D0006(6)          element address
         MVC   0(1,6),S0013        literal move, space padded
T0024    DS    0H
* MOVE Q -> SPOT
         LA    6,1                 subscript-1
         MH    6,H0003             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    0,1                 subscript-1
         AR    6,0                 add this dimension
         LA    6,D0006(6)          element address
         MVC   0(1,6),S0014        literal move, space padded
T0025    DS    0H
* MOVE Z -> SPOT
         LA    6,0                 subscript-1
         MH    6,H0003             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    6,0                 add this dimension
         LA    0,0                 subscript-1
         AR    6,0                 add this dimension
         LA    6,D0006(6)          element address
         MVC   0(1,6),S0015        literal move, space padded
T0026    DS    0H
* MOVE SPOT -> ONE-SPOT
         LA    7,0                 subscript-1
         MH    7,H0003             times element size
         LA    0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    0,0                 subscript-1
         AR    7,0                 add this dimension
         LA    7,D0006(7)          element address
         MVC   D0011(1),0(7)       alphanumeric move
T0027    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0016
         MVC   DSPBUF+4(1),D0011
         LA    1,PARM0008
         L     15,VDISP
         BALR  14,15
T0028    DS    0H
* MOVE SPOT -> ONE-SPOT
         LA    7,1                 subscript-1
         MH    7,H0003             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    0,1                 subscript-1
         AR    7,0                 add this dimension
         LA    7,D0006(7)          element address
         MVC   D0011(1),0(7)       alphanumeric move
T0029    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0017
         MVC   DSPBUF+4(1),D0011
         LA    1,PARM0009
         L     15,VDISP
         BALR  14,15
T0030    DS    0H
* MOVE SPOT -> ONE-SPOT
         LA    7,0                 subscript-1
         MH    7,H0003             times element size
         LA    0,1                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    0,0                 subscript-1
         AR    7,0                 add this dimension
         LA    7,D0006(7)          element address
         MVC   D0011(1),0(7)       alphanumeric move
T0031    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0018
         MVC   DSPBUF+4(1),D0011
         LA    1,PARM0010
         L     15,VDISP
         BALR  14,15
T0032    DS    0H
* MOVE 1 -> I
         ZAP   PWK1(16),K0004(16)  literal
         UNPK  D0007(1),PWK1(16)   packed -> zoned
         OI    D0007+0,X'F0'       unsigned: force an F zone
T0033    DS    0H
* MOVE 2 -> J
         ZAP   PWK1(16),K0001(16)  literal
         UNPK  D0008(1),PWK1(16)   packed -> zoned
         OI    D0008+0,X'F0'       unsigned: force an F zone
T0034    DS    0H
* MOVE 1 -> K
         ZAP   PWK1(16),K0004(16)  literal
         UNPK  D0009(1),PWK1(16)   packed -> zoned
         OI    D0009+0,X'F0'       unsigned: force an F zone
T0035    DS    0H
* MOVE SPOT -> ONE-SPOT
         PACK  DWK(8),D0007(1)     subscript
         CVB   7,DWK
         BCTR  7,0                 subscript-1
         MH    7,H0003             times element size
         PACK  DWK(8),D0008(1)     subscript
         CVB   0,DWK
         BCTR  0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         PACK  DWK(8),D0009(1)     subscript
         CVB   0,DWK
         BCTR  0,0                 subscript-1
         AR    7,0                 add this dimension
         LA    7,D0006(7)          element address
         MVC   D0011(1),0(7)       alphanumeric move
T0036    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0019
         MVC   DSPBUF+4(1),D0011
         LA    1,PARM0011
         L     15,VDISP
         BALR  14,15
T0037    DS    0H
* MOVE ROW -> WHOLE-ROW
         LA    7,2                 subscript-1
         MH    7,H0001             times element size
         LA    7,D0001(7)          element address
         MVC   D0012(8),0(7)       alphanumeric move
T0038    DS    0H
* DISPLAY
         MVC   DSPBUF+0(5),S0020
         MVC   DSPBUF+5(8),D0012
         LA    1,PARM0012
         L     15,VDISP
         BALR  14,15
T0039    DS    0H
* MOVE 4 -> C2
         ZAP   PWK1(16),K0003(16)  literal
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK               packed -> binary
         STH   2,D0015
T0040    DS    0H
* MOVE CELL -> SEEN
         LA    7,0                 subscript-1
         MH    7,H0001             times element size
         LH    0,D0015             subscript
         BCTR  0,0                 subscript-1
         MH    0,H0002             times element size
         AR    7,0                 add this dimension
         LA    7,D0002(7)          element address
         MVC   D0010(2),0(7)       alphanumeric move
T0041    DS    0H
* DISPLAY
         MVC   DSPBUF+0(5),S0021
         MVC   DSPBUF+5(2),D0010
         LA    1,PARM0013
         L     15,VDISP
         BALR  14,15
T0042    DS    0H
* STOP RUN
         L     15,VTERM            close anything the runtime opened
         BALR  14,15
         L     13,4(13)            restore caller's save area
         LM    14,12,12(13)        restore caller's registers
         SR    15,15               return code 0
         BR    14                  return to caller
VDISP    DC    V(COBDISP)
VTERM    DC    V(COBTERM)
PARM0001 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0001)  last parameter
LEN0001  DC    H'5'
PARM0002 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0002)  last parameter
LEN0002  DC    H'5'
PARM0003 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0003)  last parameter
LEN0003  DC    H'5'
PARM0004 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0004)  last parameter
LEN0004  DC    H'5'
PARM0005 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0005)  last parameter
LEN0005  DC    H'5'
PARM0006 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0006)  last parameter
LEN0006  DC    H'5'
PARM0007 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0007)  last parameter
LEN0007  DC    H'5'
PARM0008 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0008)  last parameter
LEN0008  DC    H'5'
PARM0009 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0009)  last parameter
LEN0009  DC    H'5'
PARM0010 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0010)  last parameter
LEN0010  DC    H'5'
PARM0011 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0011)  last parameter
LEN0011  DC    H'5'
PARM0012 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0012)  last parameter
LEN0012  DC    H'13'
PARM0013 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0013)  last parameter
LEN0013  DC    H'7'
* work areas for decimal arithmetic
DWK      DS    D                   CVD/CVB doubleword
PWK1     DS    PL16
PWK2     DS    PL16
EDSRC    DS    PL16                ED source, sized to the selectors
EDWK     DS    CL64                ED pattern and result
ZWK      DS    CL24                zoned work area
MULT8    DS    PL8                 MP right operand
DIVR8    DS    PL8                 DP divisor
QTMP     DS    PL8                 DP quotient
WK0      DS    PL16                expression stack
WK1      DS    PL16
WK2      DS    PL16
WK3      DS    PL16
WK4      DS    PL16
WK5      DS    PL16
K0001    DC    PL16'2'             numeric constants
K0002    DC    PL16'3'
K0003    DC    PL16'4'
K0004    DC    PL16'1'
H0001    DC    H'8'                element sizes
H0002    DC    H'2'
H0003    DC    H'4'
S0001    DC    CL2'A1'             nonnumeric constants
S0002    DC    CL2'A4'
S0003    DC    CL2'B2'
S0004    DC    CL2'C3'
S0005    DC    CL2'C4'
S0006    DC    CL3'11 '
S0007    DC    CL3'14 '
S0008    DC    CL3'22 '
S0009    DC    CL3'33 '
S0010    DC    CL3'34 '
S0011    DC    CL3'IJ '
S0012    DC    CL3'RC '
S0013    DC    CL1'P'
S0014    DC    CL1'Q'
S0015    DC    CL1'Z'
S0016    DC    CL4'111 '
S0017    DC    CL4'222 '
S0018    DC    CL4'121 '
S0019    DC    CL4'IJK '
S0020    DC    CL5'ROW3 '
S0021    DC    CL5'C2   '
* base locator cells, one per 4096 bytes of COBWS
BL0000   DC    A(WSC0000)
DSPBUF   DS    CL121               DISPLAY line
SAVEAREA DS    18F
* program-check exit: report the source line, then let it abend
COBSPIE  DS    0H
         USING COBSPIE,15
         STM   14,12,SPIEREGS      R15 is our base on entry
         LR    9,15                keep a base across the WTO
         DROP  15
         USING COBSPIE,9
         LR    10,1                the PIE
*  the interruption code, as the digit people know it
         SR    7,7
         IC    7,7(,10)            low byte of the interruption code
         N     7,SPIE15
         LA    7,SPIEHEX(7)
         MVC   SPIECODE(1),0(7)
*  the interrupt address, as an offset into this module
         L     2,8(,10)            second word of the old PSW
         N     2,SPIEADR           leaves the instruction address
         S     2,SPIEBEG           relative to the entry point
*  the last table entry at or before it names the statement
         L     3,SPIETAB
         LH    4,SPIENUM
         SR    5,5                 no line yet
SPIELOOP LTR   4,4
         BZ    SPIEFND
         LH    6,0(,3)             this statement's offset
         CR    6,2
         BH    SPIEFND             past it: the previous one is the ans
         LH    5,2(,3)
         LA    3,4(,3)
         BCTR  4,0
         B     SPIELOOP
SPIEFND  CVD   5,SPIEDW
         UNPK  SPIELINE(5),SPIEDW+5(3)
         OI    SPIELINE+4,X'F0'
         WTO   MF=(E,SPIEWTO)      into the job log, beside the abend
*  cancel the exit and back up to the failing instruction,
*  so the abend happens for real -- same code, same dump
         SR    2,2
         IC    2,7(,10)            the interruption code
         A     2,SPIE3000
         ABEND (2),DUMP
SPIEHEX  DC    C'0123456789ABCDEF'
SPIE15   DC    F'15'
SPIE3000 DC    F'3000'
SPIEADR  DC    X'00FFFFFF'
SPIEBEG  DC    A(COBBEG)
SPIETAB  DC    A(SPIELTB)
SPIENUM  DC    H'43'               statements in the table
SPIEREGS DS    15F
SPIEDW   DS    D
SPIEWTO  WTO   'COBC370: PROGRAM CHECK 0C0 AT SOURCE LINE 00000',      X
               MF=L
SPIECODE EQU   SPIEWTO+29,1        the 0C? digit, patched above
SPIELINE EQU   SPIEWTO+46,5        the line number, likewise
* statement offsets, ascending, paired with source lines
SPIELTB  DS    0H
         DC    AL2(T0000-COBBEG),AL2(23)
         DC    AL2(T0001-COBBEG),AL2(24)
         DC    AL2(T0002-COBBEG),AL2(25)
         DC    AL2(T0003-COBBEG),AL2(26)
         DC    AL2(T0004-COBBEG),AL2(27)
         DC    AL2(T0005-COBBEG),AL2(28)
         DC    AL2(T0006-COBBEG),AL2(29)
         DC    AL2(T0007-COBBEG),AL2(30)
         DC    AL2(T0008-COBBEG),AL2(31)
         DC    AL2(T0009-COBBEG),AL2(32)
         DC    AL2(T0010-COBBEG),AL2(33)
         DC    AL2(T0011-COBBEG),AL2(34)
         DC    AL2(T0012-COBBEG),AL2(35)
         DC    AL2(T0013-COBBEG),AL2(36)
         DC    AL2(T0014-COBBEG),AL2(37)
         DC    AL2(T0015-COBBEG),AL2(38)
         DC    AL2(T0016-COBBEG),AL2(39)
         DC    AL2(T0017-COBBEG),AL2(40)
         DC    AL2(T0018-COBBEG),AL2(41)
         DC    AL2(T0019-COBBEG),AL2(42)
         DC    AL2(T0020-COBBEG),AL2(43)
         DC    AL2(T0021-COBBEG),AL2(44)
         DC    AL2(T0022-COBBEG),AL2(45)
         DC    AL2(T0023-COBBEG),AL2(46)
         DC    AL2(T0024-COBBEG),AL2(47)
         DC    AL2(T0025-COBBEG),AL2(48)
         DC    AL2(T0026-COBBEG),AL2(49)
         DC    AL2(T0027-COBBEG),AL2(50)
         DC    AL2(T0028-COBBEG),AL2(51)
         DC    AL2(T0029-COBBEG),AL2(52)
         DC    AL2(T0030-COBBEG),AL2(53)
         DC    AL2(T0031-COBBEG),AL2(54)
         DC    AL2(T0032-COBBEG),AL2(55)
         DC    AL2(T0033-COBBEG),AL2(56)
         DC    AL2(T0034-COBBEG),AL2(57)
         DC    AL2(T0035-COBBEG),AL2(58)
         DC    AL2(T0036-COBBEG),AL2(59)
         DC    AL2(T0037-COBBEG),AL2(60)
         DC    AL2(T0038-COBBEG),AL2(61)
         DC    AL2(T0039-COBBEG),AL2(62)
         DC    AL2(T0040-COBBEG),AL2(63)
         DC    AL2(T0041-COBBEG),AL2(64)
         DC    AL2(T0042-COBBEG),AL2(65)
COBWS    CSECT
WSC0000  EQU   COBWS               chunk origins
* WORKING-STORAGE
D0000    DS    0CL24               GRID (01 group)
D0001    DS    0CL8                ROW (02 group, OCCURS 3)
D0002    DC    4CL2' '             CELL PIC X(2) table
         DS    XL16                reserve the rest of a table
D0003    DS    0CL8                CUBE (01 group)
D0004    DS    0CL4                PLANE (02 group, OCCURS 2)
D0005    DS    0CL2                LINE-X (03 group, OCCURS 2)
D0006    DC    2CL1' '             SPOT PIC X(1) table
         DS    XL6                 reserve the rest of a table
D0007    DC    CL1'0'              I PIC 9(1)v0 DISP
         DS    XL7                 reserve the rest of a table
D0008    DC    CL1'0'              J PIC 9(1)v0 DISP
         DS    XL7                 reserve the rest of a table
D0009    DC    CL1'0'              K PIC 9(1)v0 DISP
         DS    XL7                 reserve the rest of a table
D0010    DC    CL2' '              SEEN PIC X(2)
         DS    XL6                 reserve the rest of a table
D0011    DC    CL1' '              ONE-SPOT PIC X(1)
         DS    XL7                 reserve the rest of a table
D0012    DC    CL8' '              WHOLE-ROW PIC X(8)
D0013    DC    HL2'0'              R PIC S9(4)v0 COMP
         DS    XL6                 reserve the rest of a table
D0014    DC    HL2'0'              C PIC S9(4)v0 COMP
         DS    XL6                 reserve the rest of a table
D0015    DC    HL2'0'              C2 PIC S9(4)v0 COMP
*---------------------------------------------------------------
* COBRT -- our runtime. Nothing here is from SYS1.COBLIB.
* DISPLAY reaches SYSOUT through QSAM directly, which is the
* same access-method path IKFCBL00 uses for its own file I/O.
* Not reentrant: MVS 3.8j batch does not require it.
*---------------------------------------------------------------
COBRT    CSECT
         ENTRY COBDISP,COBTERM,COBWRL,COBDATE,COBACC,COBUPSI,COBADV
*
* COBDISP -- write one line to SYSOUT.
*   R1 -> A(text), A(halfword length).  Opens SYSOUT on demand.
*
COBDISP  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE1+4
         LA    11,RTSAVE1
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(text)
         L     3,4(0,1)            A(length)
         LH    4,0(0,3)            length
         CLI   RTOPEN,X'01'        already open?
         BE    COBD010
         OPEN  (RTDCB,OUTPUT)
         MVI   RTOPEN,X'01'
COBD010  MVI   RTLINE,C' '         ASA: single space
         MVC   RTLINE+1(120),RTLINE  blank the text area
         LTR   4,4
         BNP   COBD020             empty: emit a blank line
         CH    4,RTMAX
         BNH   COBD015
         LH    4,RTMAX             truncate at line width
COBD015  BCTR  4,0                 EX wants length-1
         EX    4,COBDMVC
COBD020  PUT   RTDCB,RTLINE
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
COBDMVC  MVC   RTLINE+1(0),0(2)    executed, never fallen into
*
* COBADV -- write one line with ASA carriage control.
*
*   R1 -> A(dcb), A(print buffer), A(halfword record length),
*         A(halfword owed), A(halfword request)
*
* ASA says what to do BEFORE a line prints, which is exactly
* what AFTER ADVANCING means. BEFORE has to be held over: the
* line goes out with whatever was owed from the last BEFORE,
* and its own count becomes what the next line owes. Once the
* two can add up the total is not known until run time, which
* is why this is a routine and not a few instructions inline.
*
* The request is the line count, or -1 for PAGE, negated when
* the phrase was BEFORE.
COBADV   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE7+4
         LA    11,RTSAVE7
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(dcb)
         L     3,4(0,1)            A(buffer)
         L     4,8(0,1)            A(length)
         L     5,12(0,1)           A(owed)
         L     6,16(0,1)           A(request)
         LH    7,0(0,4)            the record length
         LTR   7,7
         LH    8,0(0,6)            the request
         LH    9,0(0,5)            what the last BEFORE left owing
         LTR   8,8                 BEFORE is the negative side
         BM    ADV100
         CH    8,ADVPAGE           AFTER PAGE?
         BE    ADV020
         CH    9,ADVPAGE           was a page already owed?
         BE    ADV030              then it stays a page, whatever this
         AR    9,8                 owed plus this one
         B     ADV030
ADV020   LH    9,ADVPAGE           a page skip swallows what was owed
ADV030   XC    0(2,5),0(5)         nothing owed after an AFTER
         B     ADV200
ADV100   LCR   8,8                 back to a positive request
         CH    8,ADVPAGE           BEFORE PAGE?
         BNE   ADV110
         LH    8,ADVPAGE
ADV110   STH   8,0(0,5)            this is what the next line owes
         LTR   9,9                 nothing owed?
         BNZ   ADV200
         LH    9,ADVONE            then this line simply takes the next
ADV200   CH    9,ADVPAGE           a page skip?
         BNE   ADV210
         MVI   0(3),C'1'           skip to a new page
         B     ADV300
ADV210   LTR   9,9
         BNM   ADV220
         SR    9,9                 never negative here
ADV220   CH    9,ADVTHREE          more than one code can carry?
         BNH   ADV240
         PUT   (2),ADVB3           three blank lines at a time
         SH    9,ADVTHREE
         B     ADV220
ADV240   LA    10,ADVCODE
         AR    10,9
         MVC   0(1,3),0(10)        '+', ' ', '0' or '-'
ADV300   PUT   (2),(3)             the line itself
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
ADVB3    DC    C'-'                a blank line that advances three
         DC    CL132' '
ADVCODE  DC    C'+ 0-'             0, 1, 2 or 3 lines
ADVONE   DC    H'1'
ADVPAGE  DC    H'999'              the page-skip request
ADVTHREE DC    H'3'
RTSAVE7  DS    18F
*
* COBUPSI -- set the eight switches from the EXEC PARM.
*
* PARM='/UPSI(10100000)' is the form IBM's later compilers
* take, and the one the runtime looks for: the literal UPSI
* anywhere in the parameter text, then the next eight 0 or 1
* characters, leftmost being UPSI-0. Anything else leaves all
* eight off, which is the documented default.
COBUPSI  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE6+4
         LA    11,RTSAVE6
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(parameter text)
         LA    2,0(0,2)            drop the end-of-list bit
         XC    RTUPSI,RTUPSI       all off unless the PARM says otherwi
         LH    3,0(0,2)            its length
         LA    4,2(0,2)            the text itself
         CH    3,UPSIMIN           room for UPSI and eight digits?
         BL    UPSX
         SH    3,UPSIMIN
         LA    3,1(3)              positions worth trying
UPS10    CLC   0(4,4),UPSITAG
         BE    UPS15
         LA    4,1(4)
         BCT   3,UPS10
         B     UPSX                no UPSI in the parameter
UPS15    LA    4,4(4)              past the tag
         CLI   0(4),C'0'
         BE    UPS20
         CLI   0(4),C'1'
         BE    UPS20
         LA    4,1(4)              skip a ( or an =
UPS20    SR    5,5                 the byte being built
         LA    6,8                 eight of them
UPS30    SLL   5,1
         CLI   0(4),C'1'
         BNE   UPS40
         LA    5,1(5)
UPS40    LA    4,1(4)
         BCT   6,UPS30
         ST    5,RTUPSI            the byte, for the caller to store
UPSX     L     13,4(13)
         L     15,RTUPSI           the byte, while R12 is still ours
         ST    15,16(13)           into the save area's R15 slot
         LM    14,12,12(13)
         BR    14
*
* COBACC -- one transfer from SYSIN into the caller's item.
*
* General rule 2 on II-53 leaves the size of a transfer to the
* implementor: here it is one 80-column record. The buffer is
* blanked first, so a receiver wider than a card is padded and
* a read past the last card returns spaces rather than the
* card before it.
*
* Parameter list: A(item), A(halfword length).
COBACC   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE5+4
         LA    11,RTSAVE5
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(item)
         L     3,4(0,1)            A(length)
         LH    4,0(0,3)            length
         MVI   ACCBUF,C' '
         MVC   ACCBUF+1(255),ACCBUF  blank the buffer
         CLI   ACCEOFF,X'01'       already at end of file?
         BE    COBA020
         CLI   ACCOPEN,X'01'       already open?
         BE    COBA010
         OPEN  (ACCDCB,INPUT)
         MVI   ACCOPEN,X'01'
COBA010  LA    1,COBA030
         STCM  1,7,ACCDCB+33       into DCBEODAD
         GET   ACCDCB,ACCBUF       one card
         B     COBA020
COBA030  MVI   ACCEOFF,X'01'       end of file: the buffer stays blank
COBA020  LTR   4,4
         BNP   COBA040             nothing to store
         CH    4,ACCMAX
         BNH   COBA035
         LH    4,ACCMAX            one MVC is 256 bytes
COBA035  BCTR  4,0                 EX wants length-1
         EX    4,COBAMVC
COBA040  L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
COBAMVC  MVC   0(0,2),ACCBUF       patched by EX
*
* COBTERM -- close SYSOUT if COBDISP ever opened it.
*
COBTERM  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE2+4
         LA    11,RTSAVE2
         ST    11,8(13)
         LR    13,11
         CLI   ACCOPEN,X'01'
         BNE   COBT005
         CLOSE (ACCDCB)
         MVI   ACCOPEN,X'00'
COBT005  CLI   RTOPEN,X'01'
         BNE   COBT010
         CLOSE (RTDCB)
         MVI   RTOPEN,X'00'
COBT010  L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
*
* COBDATE -- fill an 8-byte field with MM/DD/YY.
*   R1 -> the field.  TIME DEC hands back the date as packed
*   00YYDDDF, which is Julian, so the day of year has to be
*   walked into a month and a day.
*
COBDATE  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE4+4
         LA    11,RTSAVE4
         ST    11,8(13)
         LR    13,11
         LR    2,1                 the field, before TIME takes R1
         TIME  DEC                 R0 = HHMMSSth, R1 = 00YYDDDF
         ST    1,DTPACK
         UNPK  DTZONE(7),DTPACK(4)  '00YYDDD'
         OI    DTZONE+6,X'F0'
         PACK  DTDW(8),DTZONE+4(3)
         CVB   3,DTDW              day of the year
         PACK  DTDW(8),DTZONE+2(2)
         CVB   4,DTDW              the year
         N     4,DTF3              zero if a leap year -- 1901-2099, so
*                             mod 4 is the whole rule
         LA    5,1                 month
         LA    6,DTMON
DT010    SR    7,7
         IC    7,0(0,6)            days in this month
         CH    5,DTH2              February?
         BNE   DT020
         LTR   4,4
         BNZ   DT020
         LA    7,1(0,7)            29 this year
DT020    CR    3,7
         BNH   DT030               the day falls in this month
         SR    3,7
         LA    5,1(0,5)
         LA    6,1(0,6)
         B     DT010
DT030    CVD   5,DTDW
         UNPK  0(2,2),DTDW+6(2)    MM
         OI    1(2),X'F0'
         MVI   2(2),C'/'
         CVD   3,DTDW
         UNPK  3(2,2),DTDW+6(2)    DD
         OI    4(2),X'F0'
         MVI   5(2),C'/'
         MVC   6(2,2),DTZONE+2     YY, already zoned
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
DTMON    DC    AL1(31,28,31,30,31,30,31,31,30,31,30,31)
DTF3     DC    F'3'
DTH2     DC    H'2'
DTPACK   DS    F
DTZONE   DS    CL8
DTDW     DS    D
RTSAVE4  DS    18F
*
* COBWRL -- advance the paper and write one report line.
*   R1 -> A(dcb), A(current line), A(target line), A(buffer)
*
COBWRL   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE3+4
         LA    11,RTSAVE3
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(dcb)
         L     3,4(0,1)            A(current line)
         L     4,8(0,1)            A(target line)
         L     5,12(0,1)           A(buffer)
         LH    6,0(0,3)
         LH    7,0(0,4)
COBW010  LA    8,1(0,6)
         CR    8,7                 already at the line before the targe
         BNL   COBW020
         PUT   (2),RTBLNK          skip a line
         LA    6,1(0,6)
         B     COBW010
COBW020  PUT   (2),(5)
         STH   7,0(0,3)            current line = target
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
RTBLNK   DC    CL133' '            a blank line, ASA single space
RTSAVE3  DS    18F
RTOPEN   DC    X'00'
RTMAX    DC    H'120'
RTLINE   DC    CL121' '            ASA byte + 120 columns
RTSAVE1  DS    18F
RTSAVE2  DS    18F
RTSAVE5  DS    18F
RTSAVE6  DS    18F
RTUPSI   DS    F
UPSITAG  DC    C'UPSI'
UPSIMIN  DC    H'12'               UPSI, a bracket, eight digits
ACCOPEN  DC    X'00'
ACCEOFF  DC    X'00'
ACCBUF   DC    CL256' '            one card from SYSIN, blank padded
ACCMAX   DC    H'256'
ACCDCB   DCB   DDNAME=SYSIN,DSORG=PS,MACRF=(GM),                       X
               EODAD=0
RTDCB    DCB   DDNAME=SYSOUT,DSORG=PS,MACRF=(PM),RECFM=FBA,            X
               LRECL=121,BLKSIZE=121
         END
