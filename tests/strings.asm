*---------------------------------------------------------------
* Generated by cobc370 from strings.cbl
* Standard OS/360 entry linkage. SYS1.COBLIB is never referenced.
* DISPLAY and report output are served by our own COBRT runtime.
*---------------------------------------------------------------
STRINGS  CSECT
         STM   14,12,12(13)        save caller's registers
         BALR  12,0                first code base
COBBEG   EQU   *
         USING COBBEG,12
         LA    11,2048(,12)        second code base
         LA    11,2048(,11)
         USING COBBEG+4096,11
         LA    10,2048(,11)        third code base
         LA    10,2048(,10)
         USING COBBEG+8192,10
         ST    13,SAVEAREA+4       backward chain to caller
         LA    0,SAVEAREA
         ST    0,8(13)             forward chain from caller
         LR    13,0                our save area is now current
         SR    0,0
         SPM   0                   no overflow interrupts: high-order t
         SPIE  COBSPIE,((1,7),9,(11,12),15)  report program checks by l
* MAIN.
P0000    DS    0H
T0000    DS    0H
* MOVE ALL literal -> R
         L     8,BL0000            base locator
         USING WSC0000,8
         LA    1,D0003             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(19,1),0(1)        propagate across the item
T0001    DS    0H
* STRING ... INTO R
         LA    1,D0003             the receiver
         ST    1,SB0002
         LA    1,D0000             FIRST-NM
         ST    1,SB0002+12
         LA    1,D0001             LAST-NM
         ST    1,SB0002+36
         LA    1,SB0002
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LTR   15,15               overflow?
T0002    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0010
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(20),D0003
         MVC   DSPBUF+24(1),S0011
         LA    1,PARM0001
         L     15,VDISP
         BALR  14,15
T0003    DS    0H
* MOVE ALL literal -> R
         LA    1,D0003             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(19,1),0(1)        propagate across the item
T0004    DS    0H
* STRING ... INTO R
         LA    1,D0003             the receiver
         ST    1,SB0005
         LA    1,D0002             CITY
         ST    1,SB0005+12
         LA    1,SB0005
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LTR   15,15               overflow?
T0005    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0012
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(20),D0003
         MVC   DSPBUF+24(1),S0011
         LA    1,PARM0002
         L     15,VDISP
         BALR  14,15
T0006    DS    0H
* MOVE ALL literal -> R
         LA    1,D0003             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(19,1),0(1)        propagate across the item
T0007    DS    0H
* MOVE 3 -> P
         ZAP   PWK1(16),K0001(16)  literal
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
T0008    DS    0H
* STRING ... INTO R
         LA    1,D0003             the receiver
         ST    1,SB0009
         PACK  PWK1(16),D0005(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,SP0009            P
         LA    1,SB0009
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SP0009
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         DROP  8
T0009    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0013
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(20),D0003
         MVC   DSPBUF+24(4),S0014
         MVC   DSPBUF+28(2),D0005
         LA    1,PARM0003
         L     15,VDISP
         BALR  14,15
T0010    DS    0H
* STRING ... INTO R
         LA    1,D0003             the receiver
         ST    1,SB0011
         PACK  PWK1(16),D0005(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,SP0011            P
         LA    1,SB0011
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SP0011
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         DROP  8
T0011    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0015
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(20),D0003
         MVC   DSPBUF+24(4),S0014
         MVC   DSPBUF+28(2),D0005
         LA    1,PARM0004
         L     15,VDISP
         BALR  14,15
T0012    DS    0H
* MOVE ALL literal -> R6
         LA    1,D0004             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0013    DS    0H
* MOVE 1 -> P
         ZAP   PWK1(16),K0002(16)  literal
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
T0014    DS    0H
* STRING ... INTO R6
         LA    1,D0004             the receiver
         ST    1,SB0015
         PACK  PWK1(16),D0005(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,SP0015            P
         LA    1,SB0015
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SP0015
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         BZ    L0001               no: past the ON OVERFLOW statements
         DROP  8
T0015    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0016
         LA    1,PARM0005
         L     15,VDISP
         BALR  14,15
L0001    DS    0H
T0016    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0017
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0004
         MVC   DSPBUF+10(4),S0014
         MVC   DSPBUF+14(2),D0005
         LA    1,PARM0006
         L     15,VDISP
         BALR  14,15
T0017    DS    0H
* MOVE ALL literal -> R6
         LA    1,D0004             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0018    DS    0H
* MOVE 0 -> P
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
T0019    DS    0H
* STRING ... INTO R6
         LA    1,D0004             the receiver
         ST    1,SB0021
         PACK  PWK1(16),D0005(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,SP0021            P
         LA    1,SB0021
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SP0021
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0005(2),PWK1(16)   packed -> zoned
         OI    D0005+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         BZ    L0002               no: past the ON OVERFLOW statements
         DROP  8
T0020    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0018
         LA    1,PARM0007
         L     15,VDISP
         BALR  14,15
L0002    DS    0H
T0021    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0019
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0004
         MVC   DSPBUF+10(4),S0014
         MVC   DSPBUF+14(2),D0005
         LA    1,PARM0008
         L     15,VDISP
         BALR  14,15
T0022    DS    0H
* MOVE ALL literal -> R6
         LA    1,D0004             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0023    DS    0H
* STRING ... INTO R6
         LA    1,D0004             the receiver
         ST    1,SB0026
         LA    1,SB0026
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LTR   15,15               overflow?
T0024    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0020
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0004
         MVC   DSPBUF+10(1),S0011
         LA    1,PARM0009
         L     15,VDISP
         BALR  14,15
T0025    DS    0H
* MOVE AAAA -> E
         LA    6,0                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0007(6)          element address
         MVC   0(4,6),S0021        literal move, space padded
T0026    DS    0H
* MOVE BB   -> E
         LA    6,1                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0007(6)          element address
         MVC   0(4,6),S0022        literal move, space padded
T0027    DS    0H
* MOVE CCCC -> E
         LA    6,2                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0007(6)          element address
         MVC   0(4,6),S0023        literal move, space padded
T0028    DS    0H
* MOVE ALL literal -> R
         LA    1,D0003             ALL literal
         MVC   0(1,1),S0009        the unit
         MVC   1(19,1),0(1)        propagate across the item
T0029    DS    0H
* STRING ... INTO R
         LA    1,D0003             the receiver
         ST    1,SB0032
         LA    6,1                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0007(6)          element address
         LA    1,0(,6)             E
         ST    1,SB0032+12
         LA    6,2                 subscript-1
         MH    6,H0001             times element size
         LA    6,D0007(6)          element address
         LA    1,0(,6)             E
         ST    1,SB0032+24
         LA    1,SB0032
         L     15,VSTR
         BALR  14,15               COBSTR
         DROP  8
         LTR   15,15               overflow?
T0030    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0024
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(20),D0003
         MVC   DSPBUF+24(1),S0011
         LA    1,PARM0010
         L     15,VDISP
         BALR  14,15
T0031    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0032    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0033    DS    0H
* MOVE ALL literal -> A3
         LA    1,D0012             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0034    DS    0H
* MOVE ALL literal -> A4
         LA    1,D0013             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(2,1),0(1)         propagate across the item
T0035    DS    0H
* UNSTRING S ...
         LA    1,D0008             the sending item
         ST    1,UB0038
         LA    1,D0010             A1
         ST    1,UB0038+32
         LA    1,D0011             A2
         ST    1,UB0038+48
         LA    1,D0012             A3
         ST    1,UB0038+64
         LA    1,D0013             A4
         ST    1,UB0038+80
         LA    1,UB0038
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         LTR   3,3                 overflow?
T0036    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0026
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+3(6),D0010
         MVC   DSPBUF+9(1),S0027
         MVC   DSPBUF+10(6),D0011
         MVC   DSPBUF+16(1),S0027
         MVC   DSPBUF+17(6),D0012
         MVC   DSPBUF+23(1),S0027
         MVC   DSPBUF+24(3),D0013
         LA    1,PARM0011
         L     15,VDISP
         BALR  14,15
T0037    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0038    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0039    DS    0H
* MOVE ALL literal -> A3
         LA    1,D0012             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0040    DS    0H
* MOVE ALL literal -> A4
         LA    1,D0013             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(2,1),0(1)         propagate across the item
T0041    DS    0H
* MOVE ALL literal -> D1
         LA    1,D0014             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(1,1),0(1)         propagate across the item
T0042    DS    0H
* MOVE ALL literal -> D2
         LA    1,D0015             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(1,1),0(1)         propagate across the item
T0043    DS    0H
* MOVE 0 -> C1
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0016(2),PWK1(16)   packed -> zoned
         OI    D0016+1,X'F0'       unsigned: force an F zone
T0044    DS    0H
* MOVE 0 -> C2
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0017(2),PWK1(16)   packed -> zoned
         OI    D0017+1,X'F0'       unsigned: force an F zone
T0045    DS    0H
* MOVE 0 -> TL
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0018(2),PWK1(16)   packed -> zoned
         OI    D0018+1,X'F0'       unsigned: force an F zone
T0046    DS    0H
* UNSTRING S2 ...
         LA    1,D0009             the sending item
         ST    1,UB0049
         LA    1,D0010             A1
         ST    1,UB0049+40
         LA    1,D0014             D1
         ST    1,UB0049+48
         LA    1,D0011             A2
         ST    1,UB0049+56
         LA    1,D0015             D2
         ST    1,UB0049+64
         LA    1,D0012             A3
         ST    1,UB0049+72
         PACK  PWK1(16),D0018(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,ST0049            TL
         LA    1,UB0049
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,ST0049
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0018(2),PWK1(16)   packed -> zoned
         OI    D0018+1,X'F0'       unsigned: force an F zone
         L     2,SC0049A
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         UNPK  D0016(2),PWK1(16)   packed -> zoned
         OI    D0016+1,X'F0'       unsigned: force an F zone
         L     2,SC0049B
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         UNPK  D0017(2),PWK1(16)   packed -> zoned
         OI    D0017+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         DROP  8
T0047    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0028
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+3(6),D0010
         MVC   DSPBUF+9(1),S0027
         MVC   DSPBUF+10(6),D0011
         MVC   DSPBUF+16(1),S0027
         MVC   DSPBUF+17(6),D0012
         MVC   DSPBUF+23(1),S0027
         LA    1,PARM0012
         L     15,VDISP
         BALR  14,15
T0048    DS    0H
* DISPLAY
         MVC   DSPBUF+0(5),S0029
         MVC   DSPBUF+5(2),D0014
         MVC   DSPBUF+7(1),S0027
         MVC   DSPBUF+8(2),D0015
         MVC   DSPBUF+10(1),S0027
         LA    1,PARM0013
         L     15,VDISP
         BALR  14,15
T0049    DS    0H
* DISPLAY
         MVC   DSPBUF+0(5),S0030
         MVC   DSPBUF+5(2),D0016
         MVC   DSPBUF+7(1),S0001
         MVC   DSPBUF+8(2),D0017
         MVC   DSPBUF+10(3),S0031
         MVC   DSPBUF+13(2),D0018
         LA    1,PARM0014
         L     15,VDISP
         BALR  14,15
T0050    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0051    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0052    DS    0H
* MOVE ALL literal -> A3
         LA    1,D0012             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0053    DS    0H
* UNSTRING S ...
         LA    1,D0008             the sending item
         ST    1,UB0056
         LA    1,D0010             A1
         ST    1,UB0056+24
         LA    1,D0011             A2
         ST    1,UB0056+40
         LA    1,D0012             A3
         ST    1,UB0056+56
         LA    1,UB0056
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         LTR   3,3                 overflow?
T0054    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0032
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0010
         MVC   DSPBUF+10(2),S0033
         MVC   DSPBUF+12(6),D0011
         MVC   DSPBUF+18(2),S0033
         MVC   DSPBUF+20(6),D0012
         MVC   DSPBUF+26(1),S0011
         LA    1,PARM0015
         L     15,VDISP
         BALR  14,15
T0055    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0056    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0057    DS    0H
* MOVE 7 -> UPT
         ZAP   PWK1(16),K0004(16)  literal
         UNPK  D0019(2),PWK1(16)   packed -> zoned
         OI    D0019+1,X'F0'       unsigned: force an F zone
T0058    DS    0H
* MOVE 0 -> TL
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0018(2),PWK1(16)   packed -> zoned
         OI    D0018+1,X'F0'       unsigned: force an F zone
T0059    DS    0H
* UNSTRING S ...
         LA    1,D0008             the sending item
         ST    1,UB0062
         LA    1,D0010             A1
         ST    1,UB0062+32
         LA    1,D0011             A2
         ST    1,UB0062+48
         PACK  PWK1(16),D0019(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,SP0062            UPT
         PACK  PWK1(16),D0018(2)   zoned -> packed
         ZAP   DWK(8),PWK1(16)
         CVB   2,DWK
         ST    2,ST0062            TL
         LA    1,UB0062
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SP0062
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0019(2),PWK1(16)   packed -> zoned
         OI    D0019+1,X'F0'       unsigned: force an F zone
         L     2,ST0062
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         UNPK  D0018(2),PWK1(16)   packed -> zoned
         OI    D0018+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         BZ    L0003               no: past the ON OVERFLOW statements
         DROP  8
T0060    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0034
         LA    1,PARM0016
         L     15,VDISP
         BALR  14,15
L0003    DS    0H
T0061    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0035
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0010
         MVC   DSPBUF+10(2),S0033
         MVC   DSPBUF+12(6),D0011
         MVC   DSPBUF+18(4),S0014
         MVC   DSPBUF+22(2),D0019
         MVC   DSPBUF+24(3),S0031
         MVC   DSPBUF+27(2),D0018
         LA    1,PARM0017
         L     15,VDISP
         BALR  14,15
T0062    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0063    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0064    DS    0H
* MOVE ALL literal -> A3
         LA    1,D0012             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0065    DS    0H
* UNSTRING S2 ...
         LA    1,D0009             the sending item
         ST    1,UB0069
         LA    1,D0010             A1
         ST    1,UB0069+32
         LA    1,D0011             A2
         ST    1,UB0069+48
         LA    1,D0012             A3
         ST    1,UB0069+64
         LA    1,UB0069
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         LTR   3,3                 overflow?
         BZ    L0004               no: past the ON OVERFLOW statements
T0066    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0036
         LA    1,PARM0018
         L     15,VDISP
         BALR  14,15
L0004    DS    0H
T0067    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0037
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(6),D0010
         MVC   DSPBUF+10(2),S0033
         MVC   DSPBUF+12(6),D0011
         MVC   DSPBUF+18(2),S0033
         MVC   DSPBUF+20(6),D0012
         MVC   DSPBUF+26(1),S0011
         LA    1,PARM0019
         L     15,VDISP
         BALR  14,15
T0068    DS    0H
* MOVE ALL literal -> A1
         LA    1,D0010             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0069    DS    0H
* MOVE ALL literal -> A2
         LA    1,D0011             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0070    DS    0H
* MOVE ALL literal -> A3
         LA    1,D0012             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(5,1),0(1)         propagate across the item
T0071    DS    0H
* MOVE ALL literal -> A4
         LA    1,D0013             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(2,1),0(1)         propagate across the item
T0072    DS    0H
* UNSTRING XY ...
         LA    1,D0020             the sending item
         ST    1,UB0077
         LA    1,D0010             A1
         ST    1,UB0077+32
         LA    1,D0011             A2
         ST    1,UB0077+48
         LA    1,D0012             A3
         ST    1,UB0077+64
         LA    1,D0013             A4
         ST    1,UB0077+80
         LA    1,UB0077
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         LTR   3,3                 overflow?
         BZ    L0005               no: past the ON OVERFLOW statements
T0073    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0038
         LA    1,PARM0020
         L     15,VDISP
         BALR  14,15
L0005    DS    0H
T0074    DS    0H
* DISPLAY
         MVC   DSPBUF+0(3),S0039
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+3(6),D0010
         MVC   DSPBUF+9(1),S0027
         MVC   DSPBUF+10(6),D0011
         MVC   DSPBUF+16(1),S0027
         MVC   DSPBUF+17(6),D0012
         MVC   DSPBUF+23(1),S0027
         MVC   DSPBUF+24(3),D0013
         LA    1,PARM0021
         L     15,VDISP
         BALR  14,15
T0075    DS    0H
* MOVE ALL literal -> A4
         LA    1,D0013             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(2,1),0(1)         propagate across the item
T0076    DS    0H
* MOVE ALL literal -> D1
         LA    1,D0014             ALL literal
         MVC   0(1,1),S0025        the unit
         MVC   1(1,1),0(1)         propagate across the item
T0077    DS    0H
* MOVE 0 -> C1
         ZAP   PWK1(16),K0003(16)  literal
         UNPK  D0016(2),PWK1(16)   packed -> zoned
         OI    D0016+1,X'F0'       unsigned: force an F zone
T0078    DS    0H
* UNSTRING S ...
         LA    1,D0008             the sending item
         ST    1,UB0084
         LA    1,D0013             A4
         ST    1,UB0084+32
         LA    1,D0014             D1
         ST    1,UB0084+40
         LA    1,UB0084
         L     15,VUNS
         BALR  14,15               COBUNS
         DROP  8
         LR    3,15                keep the overflow flag
         L     2,SC0084A
         CVD   2,DWK
         ZAP   PWK1(16),DWK(8)
         L     8,BL0000            base locator
         USING WSC0000,8
         UNPK  D0016(2),PWK1(16)   packed -> zoned
         OI    D0016+1,X'F0'       unsigned: force an F zone
         LTR   3,3                 overflow?
         DROP  8
T0079    DS    0H
* DISPLAY
         MVC   DSPBUF+0(4),S0040
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+4(3),D0013
         MVC   DSPBUF+7(5),S0041
         MVC   DSPBUF+12(2),D0014
         MVC   DSPBUF+14(4),S0042
         MVC   DSPBUF+18(2),D0016
         LA    1,PARM0022
         L     15,VDISP
         BALR  14,15
T0080    DS    0H
* STOP RUN
         L     15,VTERM            close anything the runtime opened
         BALR  14,15
         L     13,4(13)            restore caller's save area
         LM    14,12,12(13)        restore caller's registers
         SR    15,15               return code 0
         BR    14                  return to caller
VDISP    DC    V(COBDISP)
VTERM    DC    V(COBTERM)
PARM0001 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0001)  last parameter
LEN0001  DC    H'25'
PARM0002 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0002)  last parameter
LEN0002  DC    H'25'
PARM0003 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0003)  last parameter
LEN0003  DC    H'30'
PARM0004 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0004)  last parameter
LEN0004  DC    H'30'
PARM0005 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0005)  last parameter
LEN0005  DC    H'11'
PARM0006 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0006)  last parameter
LEN0006  DC    H'16'
PARM0007 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0007)  last parameter
LEN0007  DC    H'11'
PARM0008 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0008)  last parameter
LEN0008  DC    H'16'
PARM0009 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0009)  last parameter
LEN0009  DC    H'11'
PARM0010 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0010)  last parameter
LEN0010  DC    H'25'
PARM0011 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0011)  last parameter
LEN0011  DC    H'27'
PARM0012 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0012)  last parameter
LEN0012  DC    H'24'
PARM0013 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0013)  last parameter
LEN0013  DC    H'11'
PARM0014 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0014)  last parameter
LEN0014  DC    H'15'
PARM0015 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0015)  last parameter
LEN0015  DC    H'27'
PARM0016 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0016)  last parameter
LEN0016  DC    H'11'
PARM0017 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0017)  last parameter
LEN0017  DC    H'29'
PARM0018 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0018)  last parameter
LEN0018  DC    H'11'
PARM0019 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0019)  last parameter
LEN0019  DC    H'27'
PARM0020 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0020)  last parameter
LEN0020  DC    H'11'
PARM0021 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0021)  last parameter
LEN0021  DC    H'27'
PARM0022 DC    A(DSPBUF)
         DC    X'80',AL3(LEN0022)  last parameter
LEN0022  DC    H'20'
* work areas for decimal arithmetic
DWK      DS    D                   CVD/CVB doubleword
VSTR     DC    V(COBSTR)
VUNS     DC    V(COBUNS)
SB0002   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(20),AL2(3)      its length, the item count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              item, stored at run time
         DC    AL2(8),AL2(1)       length, delimiter length
         DC    AL4(S0001)          delimiter, a constant
         DC    AL4(S0001)          item, a constant
         DC    AL2(1),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
         DC    AL4(0)              item, stored at run time
         DC    AL2(8),AL2(1)       length, delimiter length
         DC    AL4(S0001)          delimiter, a constant
SB0005   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(20),AL2(2)      its length, the item count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              item, stored at run time
         DC    AL2(12),AL2(1)      length, delimiter length
         DC    AL4(S0002)          delimiter, a constant
         DC    AL4(S0003)          item, a constant
         DC    AL2(4),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SB0009   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(20),AL2(2)      its length, the item count
         DC    AL4(SP0009)         WITH POINTER cell
         DC    AL4(S0004)          item, a constant
         DC    AL2(2),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
         DC    AL4(S0005)          item, a constant
         DC    AL2(2),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SP0009   DS    F                   the POINTER as a fullword
SB0011   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(20),AL2(1)      its length, the item count
         DC    AL4(SP0011)         WITH POINTER cell
         DC    AL4(S0006)          item, a constant
         DC    AL2(2),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SP0011   DS    F                   the POINTER as a fullword
SB0015   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(6),AL2(1)       its length, the item count
         DC    AL4(SP0015)         WITH POINTER cell
         DC    AL4(S0007)          item, a constant
         DC    AL2(8),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SP0015   DS    F                   the POINTER as a fullword
SB0021   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(6),AL2(1)       its length, the item count
         DC    AL4(SP0021)         WITH POINTER cell
         DC    AL4(S0008)          item, a constant
         DC    AL2(2),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SP0021   DS    F                   the POINTER as a fullword
SB0026   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(6),AL2(1)       its length, the item count
         DC    AL4(0)              no POINTER
         DC    AL4(S0007)          item, a constant
         DC    AL2(8),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
SB0032   DS    0F                  STRING parameter block
         DC    AL4(0)              the receiver
         DC    AL2(20),AL2(2)      its length, the item count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              item, stored at run time
         DC    AL2(4),AL2(1)       length, delimiter length
         DC    AL4(S0001)          delimiter, a constant
         DC    AL4(0)              item, stored at run time
         DC    AL2(4),AL2(0)       length; DELIMITED BY SIZE
         DC    AL4(0)
UB0038   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(24),AL2(1)      its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              no TALLYING
         DC    AL2(4),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0002)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A1
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A2
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A3
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A4
         DC    AL2(3),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
UB0049   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(16),AL2(2)      its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(ST0049)         TALLYING cell
         DC    AL2(3),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0001)          delimiter, a constant
         DC    AL2(1),AL2(1)       length; ALL
         DC    AL4(S0002)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A1
         DC    AL2(6),AL2(2)       length, DELIMITER IN length
         DC    AL4(0)              DELIMITER IN, stored at run time
         DC    AL4(SC0049A)        COUNT IN cell
         DC    AL4(0)              A2
         DC    AL2(6),AL2(2)       length, DELIMITER IN length
         DC    AL4(0)              DELIMITER IN, stored at run time
         DC    AL4(SC0049B)        COUNT IN cell
         DC    AL4(0)              A3
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
ST0049   DS    F                   TALLYING as a fullword
SC0049A  DS    F                   COUNT IN as a fullword
SC0049B  DS    F                   COUNT IN as a fullword
UB0056   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(24),AL2(0)      its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              no TALLYING
         DC    AL2(3),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(0)              A1
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A2
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A3
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
UB0062   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(24),AL2(1)      its length, the delimiter count
         DC    AL4(SP0062)         WITH POINTER cell
         DC    AL4(ST0062)         TALLYING cell
         DC    AL2(2),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0002)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A1
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A2
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
SP0062   DS    F                   the POINTER as a fullword
ST0062   DS    F                   TALLYING as a fullword
UB0069   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(16),AL2(1)      its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              no TALLYING
         DC    AL2(3),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0001)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A1
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A2
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A3
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
UB0077   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(3),AL2(1)       its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              no TALLYING
         DC    AL2(4),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0002)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A1
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A2
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A3
         DC    AL2(6),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
         DC    AL4(0)              A4
         DC    AL2(3),AL2(0)       length, DELIMITER IN length
         DC    AL4(0)              no DELIMITER IN
         DC    AL4(0)              no COUNT IN
UB0084   DS    0F                  UNSTRING parameter block
         DC    AL4(0)              the sending item
         DC    AL2(24),AL2(1)      its length, the delimiter count
         DC    AL4(0)              no POINTER
         DC    AL4(0)              no TALLYING
         DC    AL2(1),AL2(0)       the receiver count
         DC    AL4(0)
         DC    AL4(S0002)          delimiter, a constant
         DC    AL2(1),AL2(0)       length
         DC    AL4(0)              A4
         DC    AL2(3),AL2(2)       length, DELIMITER IN length
         DC    AL4(0)              DELIMITER IN, stored at run time
         DC    AL4(SC0084A)        COUNT IN cell
SC0084A  DS    F                   COUNT IN as a fullword
PWK1     DS    PL16
PWK2     DS    PL16
EDSRC    DS    PL16                ED source, sized to the selectors
EDWK     DS    CL64                ED pattern and result
ZWK      DS    CL24                zoned work area
MULT8    DS    PL8                 MP right operand
DIVR8    DS    PL8                 DP divisor
QTMP     DS    PL8                 DP quotient
WK0      DS    PL16                expression stack
WK1      DS    PL16
WK2      DS    PL16
WK3      DS    PL16
WK4      DS    PL16
WK5      DS    PL16
K0001    DC    PL16'3'             numeric constants
K0002    DC    PL16'1'
K0003    DC    PL16'0'
K0004    DC    PL16'7'
H0001    DC    H'4'                element sizes
S0001    DC    CL1' '              nonnumeric constants
S0002    DC    CL1','
S0003    DC    CL4' USA'
S0004    DC    CL2'AB'
S0005    DC    CL2'CD'
S0006    DC    CL2'EF'
S0007    DC    CL8'ABCDEFGH'
S0008    DC    CL2'XY'
S0009    DC    CL1'-'
S0010    DC    CL4'S1 ['
S0011    DC    CL1']'
S0012    DC    CL4'S2 ['
S0013    DC    CL4'S3 ['
S0014    DC    CL4'] P='
S0015    DC    CL4'S4 ['
S0016    DC    CL11'S5 OVERFLOW'
S0017    DC    CL4'S5 ['
S0018    DC    CL11'S6 OVERFLOW'
S0019    DC    CL4'S6 ['
S0020    DC    CL4'S7 ['
S0021    DC    CL4'AAAA'
S0022    DC    CL4'BB  '
S0023    DC    CL4'CCCC'
S0024    DC    CL4'S8 ['
S0025    DC    CL1'*'
S0026    DC    CL3'U1 '
S0027    DC    CL1'|'
S0028    DC    CL3'U2 '
S0029    DC    CL5'U2 D='
S0030    DC    CL5'U2 C='
S0031    DC    CL3' T='
S0032    DC    CL4'U3 ['
S0033    DC    CL2']['
S0034    DC    CL11'U4 OVERFLOW'
S0035    DC    CL4'U4 ['
S0036    DC    CL11'U5 OVERFLOW'
S0037    DC    CL4'U5 ['
S0038    DC    CL11'U6 OVERFLOW'
S0039    DC    CL3'U6 '
S0040    DC    CL4'U7 ['
S0041    DC    CL5'] D=['
S0042    DC    CL4'] C='
* base locator cells, one per 4096 bytes of COBWS
BL0000   DC    A(WSC0000)
DSPBUF   DS    CL121               DISPLAY line
SAVEAREA DS    18F
* program-check exit: report the source line, then let it abend
COBSPIE  DS    0H
         USING COBSPIE,15
         STM   14,12,SPIEREGS      R15 is our base on entry
         LR    9,15                keep a base across the WTO
         DROP  15
         USING COBSPIE,9
         LR    10,1                the PIE
*  the interruption code, as the digit people know it
         SR    7,7
         IC    7,7(,10)            low byte of the interruption code
         N     7,SPIE15
         LA    7,SPIEHEX(7)
         MVC   SPIECODE(1),0(7)
*  the interrupt address, as an offset into this module
         L     2,8(,10)            second word of the old PSW
         N     2,SPIEADR           leaves the instruction address
         S     2,SPIEBEG           relative to the entry point
*  the last table entry at or before it names the statement
         L     3,SPIETAB
         LH    4,SPIENUM
         SR    5,5                 no line yet
SPIELOOP LTR   4,4
         BZ    SPIEFND
         LH    6,0(,3)             this statement's offset
         CR    6,2
         BH    SPIEFND             past it: the previous one is the ans
         LH    5,2(,3)
         LA    3,4(,3)
         BCTR  4,0
         B     SPIELOOP
SPIEFND  CVD   5,SPIEDW
         UNPK  SPIELINE(5),SPIEDW+5(3)
         OI    SPIELINE+4,X'F0'
         ST    2,SPIEDW            the offset, relative to COBBEG
         UNPK  SPIEOFF(7),SPIEDW+1(4)  low three bytes to zoned
         TR    SPIEOFF(6),SPIEHEXT  zoned to hex digits
         WTO   MF=(E,SPIEWTO)      into the job log, beside the abend
*  cancel the exit and back up to the failing instruction,
*  so the abend happens for real -- same code, same dump
         SR    2,2
         IC    2,7(,10)            the interruption code
         A     2,SPIE3000
         ABEND (2),DUMP
SPIEHEX  DC    C'0123456789ABCDEF'
SPIEHEXT DC    240X'00',C'0123456789ABCDEF'
SPIE15   DC    F'15'
SPIE3000 DC    F'3000'
SPIEADR  DC    X'00FFFFFF'
SPIEBEG  DC    A(COBBEG)
SPIETAB  DC    A(SPIELTB)
SPIENUM  DC    H'81'               statements in the table
SPIEREGS DS    15F
SPIEDW   DS    D
SPIEWTO  WTO   'COBC370: PROGRAM CHECK 0C0 LINE 00000 OFFSET 000000',  X
               MF=L
SPIECODE EQU   SPIEWTO+29,1        the 0C? digit, patched above
SPIELINE EQU   SPIEWTO+36,5        the line number, likewise
SPIEOFF  EQU   SPIEWTO+49,7        the offset from COBBEG, in hex
* statement offsets, ascending, paired with source lines
SPIELTB  DS    0H
         DC    AL2(T0000-COBBEG),AL2(33)
         DC    AL2(T0001-COBBEG),AL2(34)
         DC    AL2(T0002-COBBEG),AL2(36)
         DC    AL2(T0003-COBBEG),AL2(37)
         DC    AL2(T0004-COBBEG),AL2(38)
         DC    AL2(T0005-COBBEG),AL2(39)
         DC    AL2(T0006-COBBEG),AL2(40)
         DC    AL2(T0007-COBBEG),AL2(41)
         DC    AL2(T0008-COBBEG),AL2(42)
         DC    AL2(T0009-COBBEG),AL2(43)
         DC    AL2(T0010-COBBEG),AL2(44)
         DC    AL2(T0011-COBBEG),AL2(45)
         DC    AL2(T0012-COBBEG),AL2(46)
         DC    AL2(T0013-COBBEG),AL2(47)
         DC    AL2(T0014-COBBEG),AL2(48)
         DC    AL2(T0015-COBBEG),AL2(49)
         DC    AL2(T0016-COBBEG),AL2(50)
         DC    AL2(T0017-COBBEG),AL2(51)
         DC    AL2(T0018-COBBEG),AL2(52)
         DC    AL2(T0019-COBBEG),AL2(53)
         DC    AL2(T0020-COBBEG),AL2(54)
         DC    AL2(T0021-COBBEG),AL2(55)
         DC    AL2(T0022-COBBEG),AL2(56)
         DC    AL2(T0023-COBBEG),AL2(57)
         DC    AL2(T0024-COBBEG),AL2(58)
         DC    AL2(T0025-COBBEG),AL2(59)
         DC    AL2(T0026-COBBEG),AL2(60)
         DC    AL2(T0027-COBBEG),AL2(61)
         DC    AL2(T0028-COBBEG),AL2(62)
         DC    AL2(T0029-COBBEG),AL2(63)
         DC    AL2(T0030-COBBEG),AL2(64)
         DC    AL2(T0031-COBBEG),AL2(65)
         DC    AL2(T0032-COBBEG),AL2(65)
         DC    AL2(T0033-COBBEG),AL2(65)
         DC    AL2(T0034-COBBEG),AL2(65)
         DC    AL2(T0035-COBBEG),AL2(66)
         DC    AL2(T0036-COBBEG),AL2(67)
         DC    AL2(T0037-COBBEG),AL2(68)
         DC    AL2(T0038-COBBEG),AL2(68)
         DC    AL2(T0039-COBBEG),AL2(68)
         DC    AL2(T0040-COBBEG),AL2(68)
         DC    AL2(T0041-COBBEG),AL2(68)
         DC    AL2(T0042-COBBEG),AL2(68)
         DC    AL2(T0043-COBBEG),AL2(69)
         DC    AL2(T0044-COBBEG),AL2(69)
         DC    AL2(T0045-COBBEG),AL2(69)
         DC    AL2(T0046-COBBEG),AL2(70)
         DC    AL2(T0047-COBBEG),AL2(75)
         DC    AL2(T0048-COBBEG),AL2(76)
         DC    AL2(T0049-COBBEG),AL2(77)
         DC    AL2(T0050-COBBEG),AL2(78)
         DC    AL2(T0051-COBBEG),AL2(78)
         DC    AL2(T0052-COBBEG),AL2(78)
         DC    AL2(T0053-COBBEG),AL2(79)
         DC    AL2(T0054-COBBEG),AL2(80)
         DC    AL2(T0055-COBBEG),AL2(81)
         DC    AL2(T0056-COBBEG),AL2(81)
         DC    AL2(T0057-COBBEG),AL2(81)
         DC    AL2(T0058-COBBEG),AL2(81)
         DC    AL2(T0059-COBBEG),AL2(82)
         DC    AL2(T0060-COBBEG),AL2(84)
         DC    AL2(T0061-COBBEG),AL2(85)
         DC    AL2(T0062-COBBEG),AL2(86)
         DC    AL2(T0063-COBBEG),AL2(86)
         DC    AL2(T0064-COBBEG),AL2(86)
         DC    AL2(T0065-COBBEG),AL2(87)
         DC    AL2(T0066-COBBEG),AL2(88)
         DC    AL2(T0067-COBBEG),AL2(89)
         DC    AL2(T0068-COBBEG),AL2(90)
         DC    AL2(T0069-COBBEG),AL2(90)
         DC    AL2(T0070-COBBEG),AL2(90)
         DC    AL2(T0071-COBBEG),AL2(90)
         DC    AL2(T0072-COBBEG),AL2(91)
         DC    AL2(T0073-COBBEG),AL2(92)
         DC    AL2(T0074-COBBEG),AL2(93)
         DC    AL2(T0075-COBBEG),AL2(94)
         DC    AL2(T0076-COBBEG),AL2(94)
         DC    AL2(T0077-COBBEG),AL2(94)
         DC    AL2(T0078-COBBEG),AL2(95)
         DC    AL2(T0079-COBBEG),AL2(97)
         DC    AL2(T0080-COBBEG),AL2(98)
COBWS    CSECT
WSC0000  EQU   COBWS               chunk origins
* WORKING-STORAGE
D0000    DC    CL8'JOHN'           FIRST-NM PIC X(8)
D0001    DC    CL8'SMITH'          LAST-NM PIC X(8)
D0002    DC    CL12'SPRINGFIELD,'  CITY PIC X(12)
         DS    XL4                 reserve the rest of a table
D0003    DC    CL20' '             R PIC X(20)
         DS    XL4                 reserve the rest of a table
D0004    DC    CL6' '              R6 PIC X(6)
         DS    XL2                 reserve the rest of a table
D0005    DC    CL2'01'             P PIC 9(2)v0 DISP
         DS    XL6                 reserve the rest of a table
D0006    DS    0CL12               T (01 group)
D0007    DC    3CL4' '             E PIC X(4) table
         DS    XL4                 reserve the rest of a table
D0008    DC    CL24'ALPHA,BETA,,DELTA  EPS'  S PIC X(24)
D0009    DC    CL16'ONE   TWO,THREE'  S2 PIC X(16)
D0010    DC    CL6' '              A1 PIC X(6)
         DS    XL2                 reserve the rest of a table
D0011    DC    CL6' '              A2 PIC X(6)
         DS    XL2                 reserve the rest of a table
D0012    DC    CL6' '              A3 PIC X(6)
         DS    XL2                 reserve the rest of a table
D0013    DC    CL3' '              A4 PIC X(3)
         DS    XL5                 reserve the rest of a table
D0014    DC    CL2' '              D1 PIC X(2)
         DS    XL6                 reserve the rest of a table
D0015    DC    CL2' '              D2 PIC X(2)
         DS    XL6                 reserve the rest of a table
D0016    DC    CL2'00'             C1 PIC 9(2)v0 DISP
         DS    XL6                 reserve the rest of a table
D0017    DC    CL2'00'             C2 PIC 9(2)v0 DISP
         DS    XL6                 reserve the rest of a table
D0018    DC    CL2'00'             TL PIC 9(2)v0 DISP
         DS    XL6                 reserve the rest of a table
D0019    DC    CL2'01'             UPT PIC 9(2)v0 DISP
         DS    XL6                 reserve the rest of a table
D0020    DC    CL3'X,Y'            XY PIC X(3)
*---------------------------------------------------------------
* COBRT -- our runtime. Nothing here is from SYS1.COBLIB.
* DISPLAY reaches SYSOUT through QSAM directly, which is the
* same access-method path IKFCBL00 uses for its own file I/O.
* Not reentrant: MVS 3.8j batch does not require it.
*---------------------------------------------------------------
COBRT    CSECT
         ENTRY COBDISP,COBTERM,COBWRL,COBDATE,COBACC,COBUPSI
         ENTRY COBADV,COBSTR,COBUNS
*
* COBDISP -- write one line to SYSOUT.
*   R1 -> A(text), A(halfword length).  Opens SYSOUT on demand.
*
COBDISP  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE1+4
         LA    11,RTSAVE1
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(text)
         L     3,4(0,1)            A(length)
         LH    4,0(0,3)            length
         CLI   RTOPEN,X'01'        already open?
         BE    COBD010
         OPEN  (RTDCB,OUTPUT)
         MVI   RTOPEN,X'01'
COBD010  MVI   RTLINE,C' '         ASA: single space
         MVC   RTLINE+1(120),RTLINE  blank the text area
         LTR   4,4
         BNP   COBD020             empty: emit a blank line
         CH    4,RTMAX
         BNH   COBD015
         LH    4,RTMAX             truncate at line width
COBD015  BCTR  4,0                 EX wants length-1
         EX    4,COBDMVC
COBD020  PUT   RTDCB,RTLINE
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
COBDMVC  MVC   RTLINE+1(0),0(2)    executed, never fallen into
*
* COBADV -- write one line with ASA carriage control.
*
*   R1 -> A(dcb), A(print buffer), A(halfword record length),
*         A(halfword owed), A(halfword request)
*
* ASA says what to do BEFORE a line prints, which is exactly
* what AFTER ADVANCING means. BEFORE has to be held over: the
* line goes out with whatever was owed from the last BEFORE,
* and its own count becomes what the next line owes. Once the
* two can add up the total is not known until run time, which
* is why this is a routine and not a few instructions inline.
*
* The request is the line count, or -1 for PAGE, negated when
* the phrase was BEFORE.
COBADV   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE7+4
         LA    11,RTSAVE7
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(dcb)
         L     3,4(0,1)            A(buffer)
         L     4,8(0,1)            A(length)
         L     5,12(0,1)           A(owed)
         L     6,16(0,1)           A(request)
         LH    7,0(0,4)            the record length
         LTR   7,7
         LH    8,0(0,6)            the request
         LH    9,0(0,5)            what the last BEFORE left owing
         LTR   8,8                 BEFORE is the negative side
         BM    ADV100
         CH    8,ADVPAGE           AFTER PAGE?
         BE    ADV020
         CH    9,ADVPAGE           was a page already owed?
         BE    ADV030              then it stays a page, whatever this
         AR    9,8                 owed plus this one
         B     ADV030
ADV020   LH    9,ADVPAGE           a page skip swallows what was owed
ADV030   XC    0(2,5),0(5)         nothing owed after an AFTER
         B     ADV200
ADV100   LCR   8,8                 back to a positive request
         CH    8,ADVPAGE           BEFORE PAGE?
         BNE   ADV110
         LH    8,ADVPAGE
ADV110   STH   8,0(0,5)            this is what the next line owes
         LTR   9,9                 nothing owed?
         BNZ   ADV200
         LH    9,ADVONE            then this line simply takes the next
ADV200   CH    9,ADVPAGE           a page skip?
         BNE   ADV210
         MVI   0(3),C'1'           skip to a new page
         B     ADV300
ADV210   LTR   9,9
         BNM   ADV220
         SR    9,9                 never negative here
ADV220   CH    9,ADVTHREE          more than one code can carry?
         BNH   ADV240
         PUT   (2),ADVB3           three blank lines at a time
         SH    9,ADVTHREE
         B     ADV220
ADV240   LA    10,ADVCODE
         AR    10,9
         MVC   0(1,3),0(10)        '+', ' ', '0' or '-'
ADV300   PUT   (2),(3)             the line itself
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
ADVB3    DC    C'-'                a blank line that advances three
         DC    CL132' '
ADVCODE  DC    C'+ 0-'             0, 1, 2 or 3 lines
ADVONE   DC    H'1'
ADVPAGE  DC    H'999'              the page-skip request
ADVTHREE DC    H'3'
RTSAVE7  DS    18F
*
* COBSTR -- STRING. R1 -> AL4(receiver), AL2(len), AL2(n),
*   AL4(pointer cell or 0); then per item AL4(addr), AL2(len),
*   AL2(delimiter len, 0 for SIZE), AL4(delimiter addr).
* Each item goes byte by byte until its delimiter matches or
* it runs out; the receiver filling up is the overflow, and
* a pointer outside 1..len is overflow with nothing moved.
* R15 = 0 done, 1 overflow. Other bytes of the receiver are
* not touched, which is the rule: STRING does not space-fill.
COBSTR   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE8+4
         LA    11,RTSAVE8
         ST    11,8(13)
         LR    13,11
         LR    2,1                 the block
         L     3,0(2)              receiver
         LH    5,4(2)              its length
         LH    7,6(2)              items
         L     4,8(2)              pointer cell
         LTR   4,4
         BZ    STR010              none: start at 1
         L     4,0(4)
         B     STR020
STR010   LA    4,1
STR020   BCTR  4,0                 position, from 0
         LTR   4,4
         BM    STRBAD              pointer below 1
         CR    4,5
         BNL   STRBAD              pointer past the end
         LA    6,12(2)             the first item
STR100   LTR   7,7
         BZ    STRDONE             all items sent
         L     8,0(6)              item
         LH    9,4(6)              its length
         LH    11,6(6)             delimiter length
         L     10,8(6)             delimiter
STR110   LTR   9,9
         BZ    STR190              item exhausted
         LTR   11,11
         BZ    STR120              SIZE: no delimiter to look for
         CR    9,11
         BL    STR120              too few left to hold the delimiter
         LR    15,11
         BCTR  15,0
         EX    15,STRCLC           delimiter here?
         BE    STR190              yes: the item ends
STR120   CR    4,5
         BNL   STROVF              receiver full
         LA    15,0(3,4)
         MVC   0(1,15),0(8)        one byte
         LA    8,1(8)
         BCTR  9,0
         LA    4,1(4)
         B     STR110
STR190   LA    6,12(6)             next item
         BCTR  7,0
         B     STR100
STRDONE  SR    15,15
         B     STRRET
STROVF   LA    15,1                overflow
STRRET   L     14,8(2)             pointer cell
         LTR   14,14
         BZ    STRX
         LA    4,1(4)              back to counting from 1
         ST    4,0(14)
         B     STRX
STRBAD   LA    15,1                pointer out of range: overflow, noth
STRX     L     13,4(13)
         ST    15,16(13)           R15 survives the LM through its own
         LM    14,12,12(13)
         BR    14
STRCLC   CLC   0(0,8),0(10)        executed with the delimiter length
RTSAVE8  DS    18F
*
* COBUNS -- UNSTRING. R1 -> AL4(sender), AL2(len), AL2(nd),
*   AL4(pointer cell or 0), AL4(tally cell or 0), AL2(nr),
*   AL2(0), AL4(0); then nd delimiters AL4(addr), AL2(len),
*   AL2(ALL); then nr receivers AL4(addr), AL2(len),
*   AL2(DELIMITER IN len), AL4(DELIMITER IN or 0),
*   AL4(COUNT IN cell or 0).
* For each receiver the sender is scanned from the position
* for the first place any delimiter matches, delimiters tried
* in order; the bytes before it go to the receiver, left
* justified and space filled. No delimiters means a receiver
* takes its own length. Bytes left when the receivers are
* used up are the overflow. R15 = 0 done, 1 overflow.
COBUNS   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE9+4
         LA    11,RTSAVE9
         ST    11,8(13)
         LR    13,11
         LR    2,1                 the block
         L     3,0(2)              sender
         LH    5,4(2)              its length
         LH    10,6(2)             delimiters
         L     4,8(2)              pointer cell
         LTR   4,4
         BZ    UNS010
         L     4,0(4)
         B     UNS020
UNS010   LA    4,1
UNS020   BCTR  4,0                 position, from 0
         LTR   4,4
         BM    UNSBAD
         CR    4,5
         BNL   UNSBAD              pointer past the end
         LH    7,16(2)             receivers
         LR    6,10
         SLL   6,3                 eight bytes per delimiter
         LA    6,24(6,2)           the first receiver
UNS100   LTR   7,7
         BZ    UNSEND              receivers used up
         CR    4,5
         BNL   UNSEND              sender used up: the rest stay as the
         LR    8,4                 scan from the position
         SR    9,9                 no delimiter matched yet
         LTR   10,10
         BZ    UNS140              no delimiters: take the receiver's l
UNS110   CR    8,5
         BNL   UNS170              end of the sender, no delimiter
         LA    9,24(2)             the first delimiter
         LR    0,10
UNS120   L     14,0(9)             delimiter
         LH    15,4(9)             its length
         LR    1,5
         SR    1,8                 bytes left
         CR    1,15
         BL    UNS130              not enough left for this one
         LA    1,0(3,8)
         BCTR  15,0
         EX    15,UNSCLC           matches here?
         BE    UNS170              yes, R9 says which
UNS130   LA    9,8(9)              next delimiter
         BCT   0,UNS120
         SR    9,9                 none matched at this byte
         LA    8,1(8)
         B     UNS110
UNS140   LH    1,4(6)              receiver length
         LA    8,0(4,1)            position plus that
         CR    8,5
         BNH   UNS170
         LR    8,5                 but no further than the end
UNS170   L     14,0(6)             receiver
         LH    15,4(6)             its length
         MVI   0(14),C' '
         LR    11,15
         BCTR  11,0
         LTR   11,11
         BZ    UNS172              a one-byte receiver is filled
         BCTR  11,0
         EX    11,UNSPAD           space fill
UNS172   LR    1,8
         SR    1,4                 bytes to move
         L     11,12(6)            COUNT IN cell
         LTR   11,11
         BZ    UNS174
         ST    1,0(11)             the count, before truncation
UNS174   CR    1,15
         BNH   UNS176
         LR    1,15                truncated on the right
UNS176   LTR   1,1
         BZ    UNS180              nothing to move
         BCTR  1,0
         LA    15,0(3,4)
         EX    1,UNSMVC            the bytes
UNS180   L     14,8(6)             DELIMITER IN
         LTR   14,14
         BZ    UNS186
         LH    15,6(6)             its length
         MVI   0(14),C' '
         LR    11,15
         BCTR  11,0
         LTR   11,11
         BZ    UNS182
         BCTR  11,0
         EX    11,UNSPAD           space fill
UNS182   LTR   9,9
         BZ    UNS186              no delimiter: spaces
         LH    1,4(9)              delimiter length
         CR    1,15
         BNH   UNS184
         LR    1,15
UNS184   BCTR  1,0
         L     15,0(9)
         EX    1,UNSMVC            the delimiter
UNS186   L     14,12(2)            TALLYING cell
         LTR   14,14
         BZ    UNS188
         L     15,0(14)
         LA    15,1(15)            one more receiver acted on
         ST    15,0(14)
UNS188   LTR   9,9
         BZ    UNS196              no delimiter: position is where the
         LH    1,4(9)              delimiter length
         LA    4,0(1,8)            past the delimiter
         LH    0,6(9)              ALL?
         LTR   0,0
         BZ    UNS198
UNS190   LR    0,5
         SR    0,4                 bytes left
         CR    0,1
         BL    UNS198              not enough for another copy
         LA    15,0(3,4)
         L     14,0(9)
         LR    11,1
         BCTR  11,0
         EX    11,UNSCLC2          another copy of the delimiter?
         BNE   UNS198
         LA    4,0(1,4)            skip it too
         B     UNS190
UNS196   LR    4,8
UNS198   LA    6,16(6)             next receiver
         BCTR  7,0
         B     UNS100
UNSEND   SR    15,15
         CR    4,5
         BNL   UNS200
         LA    15,1                bytes left over: overflow
UNS200   L     14,8(2)             pointer cell
         LTR   14,14
         BZ    UNSX
         LA    4,1(4)
         ST    4,0(14)
         B     UNSX
UNSBAD   LA    15,1                pointer out of range: overflow, noth
UNSX     L     13,4(13)
         ST    15,16(13)           R15 survives the LM through its own
         LM    14,12,12(13)
         BR    14
UNSCLC   CLC   0(0,1),0(14)        executed with the delimiter length
UNSCLC2  CLC   0(0,15),0(14)
UNSMVC   MVC   0(0,14),0(15)       executed with the byte count
UNSPAD   MVC   1(0,14),0(14)       executed: propagate the space
RTSAVE9  DS    18F
*
* COBUPSI -- set the eight switches from the EXEC PARM.
*
* PARM='/UPSI(10100000)' is the form IBM's later compilers
* take, and the one the runtime looks for: the literal UPSI
* anywhere in the parameter text, then the next eight 0 or 1
* characters, leftmost being UPSI-0. Anything else leaves all
* eight off, which is the documented default.
COBUPSI  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE6+4
         LA    11,RTSAVE6
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(parameter text)
         LA    2,0(0,2)            drop the end-of-list bit
         XC    RTUPSI,RTUPSI       all off unless the PARM says otherwi
         LH    3,0(0,2)            its length
         LA    4,2(0,2)            the text itself
         CH    3,UPSIMIN           room for UPSI and eight digits?
         BL    UPSX
         SH    3,UPSIMIN
         LA    3,1(3)              positions worth trying
UPS10    CLC   0(4,4),UPSITAG
         BE    UPS15
         LA    4,1(4)
         BCT   3,UPS10
         B     UPSX                no UPSI in the parameter
UPS15    LA    4,4(4)              past the tag
         CLI   0(4),C'0'
         BE    UPS20
         CLI   0(4),C'1'
         BE    UPS20
         LA    4,1(4)              skip a ( or an =
UPS20    SR    5,5                 the byte being built
         LA    6,8                 eight of them
UPS30    SLL   5,1
         CLI   0(4),C'1'
         BNE   UPS40
         LA    5,1(5)
UPS40    LA    4,1(4)
         BCT   6,UPS30
         ST    5,RTUPSI            the byte, for the caller to store
UPSX     L     13,4(13)
         L     15,RTUPSI           the byte, while R12 is still ours
         ST    15,16(13)           into the save area's R15 slot
         LM    14,12,12(13)
         BR    14
*
* COBACC -- one transfer from SYSIN into the caller's item.
*
* General rule 2 on II-53 leaves the size of a transfer to the
* implementor: here it is one 80-column record. The buffer is
* blanked first, so a receiver wider than a card is padded and
* a read past the last card returns spaces rather than the
* card before it.
*
* Parameter list: A(item), A(halfword length).
COBACC   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE5+4
         LA    11,RTSAVE5
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(item)
         L     3,4(0,1)            A(length)
         LH    4,0(0,3)            length
         MVI   ACCBUF,C' '
         MVC   ACCBUF+1(255),ACCBUF  blank the buffer
         CLI   ACCEOFF,X'01'       already at end of file?
         BE    COBA020
         CLI   ACCOPEN,X'01'       already open?
         BE    COBA010
         OPEN  (ACCDCB,INPUT)
         MVI   ACCOPEN,X'01'
COBA010  LA    1,COBA030
         STCM  1,7,ACCDCB+33       into DCBEODAD
         GET   ACCDCB,ACCBUF       one card
         B     COBA020
COBA030  MVI   ACCEOFF,X'01'       end of file: the buffer stays blank
COBA020  LTR   4,4
         BNP   COBA040             nothing to store
         CH    4,ACCMAX
         BNH   COBA035
         LH    4,ACCMAX            one MVC is 256 bytes
COBA035  BCTR  4,0                 EX wants length-1
         EX    4,COBAMVC
COBA040  L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
COBAMVC  MVC   0(0,2),ACCBUF       patched by EX
*
* COBTERM -- close SYSOUT if COBDISP ever opened it.
*
COBTERM  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE2+4
         LA    11,RTSAVE2
         ST    11,8(13)
         LR    13,11
         CLI   ACCOPEN,X'01'
         BNE   COBT005
         CLOSE (ACCDCB)
         MVI   ACCOPEN,X'00'
COBT005  CLI   RTOPEN,X'01'
         BNE   COBT010
         CLOSE (RTDCB)
         MVI   RTOPEN,X'00'
COBT010  L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
*
* COBDATE -- fill an 8-byte field with MM/DD/YY.
*   R1 -> the field.  TIME DEC hands back the date as packed
*   00YYDDDF, which is Julian, so the day of year has to be
*   walked into a month and a day.
*
COBDATE  STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE4+4
         LA    11,RTSAVE4
         ST    11,8(13)
         LR    13,11
         LR    2,1                 the field, before TIME takes R1
         TIME  DEC                 R0 = HHMMSSth, R1 = 00YYDDDF
         ST    1,DTPACK
         UNPK  DTZONE(7),DTPACK(4)  '00YYDDD'
         OI    DTZONE+6,X'F0'
         PACK  DTDW(8),DTZONE+4(3)
         CVB   3,DTDW              day of the year
         PACK  DTDW(8),DTZONE+2(2)
         CVB   4,DTDW              the year
         N     4,DTF3              zero if a leap year -- 1901-2099, so
*                             mod 4 is the whole rule
         LA    5,1                 month
         LA    6,DTMON
DT010    SR    7,7
         IC    7,0(0,6)            days in this month
         CH    5,DTH2              February?
         BNE   DT020
         LTR   4,4
         BNZ   DT020
         LA    7,1(0,7)            29 this year
DT020    CR    3,7
         BNH   DT030               the day falls in this month
         SR    3,7
         LA    5,1(0,5)
         LA    6,1(0,6)
         B     DT010
DT030    CVD   5,DTDW
         UNPK  0(2,2),DTDW+6(2)    MM
         OI    1(2),X'F0'
         MVI   2(2),C'/'
         CVD   3,DTDW
         UNPK  3(2,2),DTDW+6(2)    DD
         OI    4(2),X'F0'
         MVI   5(2),C'/'
         MVC   6(2,2),DTZONE+2     YY, already zoned
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
DTMON    DC    AL1(31,28,31,30,31,30,31,31,30,31,30,31)
DTF3     DC    F'3'
DTH2     DC    H'2'
DTPACK   DS    F
DTZONE   DS    CL8
DTDW     DS    D
RTSAVE4  DS    18F
*
* COBWRL -- advance the paper and write one report line.
*   R1 -> A(dcb), A(current line), A(target line), A(buffer)
*
COBWRL   STM   14,12,12(13)
         BALR  12,0
         USING *,12
         ST    13,RTSAVE3+4
         LA    11,RTSAVE3
         ST    11,8(13)
         LR    13,11
         L     2,0(0,1)            A(dcb)
         L     3,4(0,1)            A(current line)
         L     4,8(0,1)            A(target line)
         L     5,12(0,1)           A(buffer)
         LH    6,0(0,3)
         LH    7,0(0,4)
COBW010  LA    8,1(0,6)
         CR    8,7                 already at the line before the targe
         BNL   COBW020
         PUT   (2),RTBLNK          skip a line
         LA    6,1(0,6)
         B     COBW010
COBW020  PUT   (2),(5)
         STH   7,0(0,3)            current line = target
         L     13,4(13)
         LM    14,12,12(13)
         SR    15,15
         BR    14
RTBLNK   DC    CL133' '            a blank line, ASA single space
RTSAVE3  DS    18F
RTOPEN   DC    X'00'
RTMAX    DC    H'120'
RTLINE   DC    CL121' '            ASA byte + 120 columns
RTSAVE1  DS    18F
RTSAVE2  DS    18F
RTSAVE5  DS    18F
RTSAVE6  DS    18F
RTUPSI   DS    F
UPSITAG  DC    C'UPSI'
UPSIMIN  DC    H'12'               UPSI, a bracket, eight digits
ACCOPEN  DC    X'00'
ACCEOFF  DC    X'00'
ACCBUF   DC    CL256' '            one card from SYSIN, blank padded
ACCMAX   DC    H'256'
ACCDCB   DCB   DDNAME=SYSIN,DSORG=PS,MACRF=(GM),                       X
               EODAD=0
RTDCB    DCB   DDNAME=SYSOUT,DSORG=PS,MACRF=(PM),RECFM=FBA,            X
               LRECL=121,BLKSIZE=121
         END
