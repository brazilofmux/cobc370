*---------------------------------------------------------------
* Generated by cobc370 from ksdsboth.cbl
...
T0000    DS    0H
* OPEN OUTPUT K-FILE
         MODCB ACB=FD000,MACRF=(KEY,SEQ,DIR,OUT,RST)  this OPEN's mode:
         LTR   15,15
         BNZ   L0021               a MODCB that failed is an OPEN that
         MODCB RPL=FD000N,OPTCD=(SEQ)  insert sequentially: load mode
         OPEN  (FD000)             VSAM ACB
...
T0003    DS    0H
* MOVE 0000000001 -> K-KEY
         MVC   D0002(10),S0002     literal move, space padded
T0004    DS    0H
* MOVE ONE -> K-TEXT
         MVC   D0003(5),S0003      literal move, space padded
T0005    DS    0H
* WRITE K-REC
         PUT   RPL=FD000N          VSAM insert by key
         MVI   FD000NA,X'01'       the RPL has carried a request
         LTR   15,15               stored?
         BNZ   L0001
         LTR   15,15               VSAM request succeeded?
         BZ    G0004
         SHOWCB RPL=FD000N,FIELDS=FDBK,AREA=VSFB,LENGTH=4  why did it f
...
G0006    DS    0H
         ...
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0005(2),=C'22'     duplicate key
         B     G0009
G0007    DS    0H
         DROP  8
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0005(2),=C'21'     key out of sequence
         B     G0009
G0008    DS    0H
         DROP  8
         L     8,BL0000            base locator
...
G0009    DS    0H
         DROP  8
         CLI   FD000NA,X'00'       ever used?
         BE    L0022
         ENDREQ RPL=FD000N         the failed request is over
         MVI   FD000NA,X'00'
L0022    DS    0H
G0005    DS    0H
         B     L0002
...
L0002    DS    0H
T0007    DS    0H
* MOVE 0000000002 -> K-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(10),S0005     literal move, space padded
T0008    DS    0H
* MOVE TWO -> K-TEXT
         MVC   D0003(5),S0006      literal move, space padded
T0009    DS    0H
* WRITE K-REC
         PUT   RPL=FD000N          VSAM insert by key
         MVI   FD000NA,X'01'       the RPL has carried a request
...
B0002    EQU   *
         USING B0002,12
T0011    DS    0H
* MOVE 0000000003 -> K-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(10),S0008     literal move, space padded
T0012    DS    0H
* MOVE THREE -> K-TEXT
         MVC   D0003(5),S0009      literal move, space padded
T0013    DS    0H
* WRITE K-REC
         PUT   RPL=FD000N          VSAM insert by key
         MVI   FD000NA,X'01'       the RPL has carried a request
...
L0006    DS    0H
T0015    DS    0H
* MOVE 0000000004 -> K-KEY
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0002(10),S0011     literal move, space padded
T0016    DS    0H
* MOVE FOUR -> K-TEXT
         MVC   D0003(5),S0012      literal move, space padded
T0017    DS    0H
* WRITE K-REC
         PUT   RPL=FD000N          VSAM insert by key
         MVI   FD000NA,X'01'       the RPL has carried a request
...
T0021    DS    0H
* OPEN I-O K-FILE
         MODCB ACB=FD000,MACRF=(KEY,SEQ,DIR,OUT,NRS)  this OPEN's mode:
         LTR   15,15
         BNZ   L0032               a MODCB that failed is an OPEN that
         MODCB RPL=FD000N,OPTCD=(DIR)  insert by key
         OPEN  (FD000)             VSAM ACB
         CH    15,VSFOUR           a warning?
         BNE   *+6
         SR    15,15               then it opened
...
T0025    DS    0H
* MOVE DEUX -> K-TEXT
         MVC   D0003(5),S0015      literal move, space padded
T0026    DS    0H
* REWRITE K-REC
...
T0027    DS    0H
* DISPLAY
         MVC   DSPBUF+0(18),S0016
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+18(2),D0005+0
         LA    1,PARM0007
         L     15,VDISP
         BALR  14,15
         DROP  8
...
T0031    DS    0H
* DISPLAY
         MVC   DSPBUF+0(17),S0018
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+17(2),D0005+0
         LA    1,PARM0009
         L     15,VDISP
         BALR  14,15
         DROP  8
...
* DISPLAY
         ...
         BALR  14,15
T0033    DS    0H
* MOVE 0000000009 -> K-KEY
         MVC   D0002(10),S0020     literal move, space padded
T0034    DS    0H
* MOVE NINE -> K-TEXT
         MVC   D0003(5),S0021      literal move, space padded
T0035    DS    0H
* REWRITE K-REC
         MVC   FD000S(80),D0001    keep the record area
         MODCB RPL=FD000R,OPTCD=(DIR,UPD)  by key, for update
...
* OPEN INPUT K-FILE
         MODCB ACB=FD000,MACRF=(KEY,SEQ,DIR,OUT,NRS)  this OPEN's mode:
         LTR   15,15
         BNZ   L0043               a MODCB that failed is an OPEN that
         MODCB RPL=FD000N,OPTCD=(DIR)  insert by key
         OPEN  (FD000)             VSAM ACB
         CH    15,VSFOUR           a warning?
         BNE   *+6
...
T0044    DS    0H
* DISPLAY
         MVC   DSPBUF+0(11),S0024
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+11(2),D0005+0
         LA    1,PARM0013
         L     15,VDISP
         BALR  14,15
T0045    DS    0H
* MOVE N -> EOF
         MVC   D0006(1),S0025      literal move, space padded
T0046    DS    0H
* READ K-FILE
         MODCB RPL=FD000R,OPTCD=(SEQ,UPD)  next
         LTR   15,15
         BZ    G0112
         MVC   D0005(2),=C'30'     could not set the request type
         B     G0113
G0112    DS    0H
         DROP  8
...
* MOVE Y -> EOF
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   D0006(1),S0026      literal move, space padded
         DROP  8
L0018    DS    0H
T0048    DS    0H
* PERFORM SHOW
L0046    DS    0H
         L     8,BL0000            base locator
         USING WSC0000,8
         CLC   D0006(1),S0026      alphanumeric compare
         BE    L0047
...
T0050    DS    0H
* DISPLAY
         MVC   DSPBUF+0(5),S0027
         L     8,BL0000            base locator
         USING WSC0000,8
         MVC   DSPBUF+5(10),D0002+0
         MVC   DSPBUF+15(1),S0028
         MVC   DSPBUF+16(5),D0003+0
         LA    1,PARM0014
         L     15,VDISP
         BALR  14,15
T0051    DS    0H
...
WK5      DS    PL16
* file control blocks
FD000    ACB   DDNAME=KSDSF01,MACRF=(KEY,SEQ,DIR,OUT,RST)  VSAM access
FD000RA  DC    F'0'                has carried a request
FD000R   RPL   ACB=FD000,AREA=D0001,                                   X
               AREALEN=80,RECLEN=80,ARG=D0002,KEYLEN=10,OPTCD=(KEY,SEQ,X
               KEQ,UPD,MVE)
FD000NA  DC    F'0'                has carried a request
FD000N   RPL   ACB=FD000,AREA=D0001,                                   X
               AREALEN=80,RECLEN=80,ARG=D0002,KEYLEN=10,OPTCD=(KEY,DIR,X
               KEQ,NUP,MVE)
FD000S   DS    CL80                the record area, across a REWRITE's
S0001    DC    CL12'OPEN OUTPUT '  nonnumeric constants
S0002    DC    CL10'0000000001'
S0003    DC    CL5'ONE  '
S0004    DC    CL16'WRITE 1 INVALID '
S0005    DC    CL10'0000000002'
S0006    DC    CL5'TWO  '
S0007    DC    CL16'WRITE 2 INVALID '
S0008    DC    CL10'0000000003'
S0009    DC    CL5'THREE'
S0010    DC    CL16'WRITE 3 INVALID '
S0011    DC    CL10'0000000004'
S0012    DC    CL5'FOUR '
S0013    DC    CL16'WRITE 4 INVALID '
S0014    DC    CL9'OPEN I-O '
S0015    DC    CL5'DEUX '
S0016    DC    CL18'REWRITE 2 INVALID '
S0017    DC    CL10'REWRITE 2 '
S0018    DC    CL17'DELETE 3 INVALID '
S0019    DC    CL9'DELETE 3 '
S0020    DC    CL10'0000000009'
S0021    DC    CL5'NINE '
S0022    DC    CL18'REWRITE 9 INVALID '
S0023    DC    CL17'DELETE 9 INVALID '
S0024    DC    CL11'OPEN INPUT '
S0025    DC    CL1'N'
S0026    DC    CL1'Y'
S0027    DC    CL5'READ '
S0028    DC    CL1' '
...
SPIENUM  DC    H'53'               statements in the table
SPIEREGS DS    15F
...
SPIELTB  DS    0F
         ...
         DC    A(T0010-COBBEG),AL2(36,0)
         DC    A(T0011-COBBEG),AL2(37,0)
         DC    A(T0012-COBBEG),AL2(37,0)
         DC    A(T0013-COBBEG),AL2(38,0)
         DC    A(T0014-COBBEG),AL2(38,0)
         DC    A(T0015-COBBEG),AL2(39,0)
         ...
         DC    A(T0025-COBBEG),AL2(46,0)
         DC    A(T0026-COBBEG),AL2(48,0)
         DC    A(T0027-COBBEG),AL2(48,0)
         DC    A(T0028-COBBEG),AL2(49,0)
         DC    A(T0029-COBBEG),AL2(50,0)
         DC    A(T0030-COBBEG),AL2(52,0)
         DC    A(T0031-COBBEG),AL2(52,0)
         DC    A(T0032-COBBEG),AL2(53,0)
         DC    A(T0033-COBBEG),AL2(54,0)
         DC    A(T0034-COBBEG),AL2(54,0)
         DC    A(T0035-COBBEG),AL2(56,0)
         DC    A(T0036-COBBEG),AL2(56,0)
         DC    A(T0037-COBBEG),AL2(57,0)
         DC    A(T0038-COBBEG),AL2(59,0)
         DC    A(T0039-COBBEG),AL2(59,0)
         DC    A(T0040-COBBEG),AL2(60,0)
         DC    A(T0041-COBBEG),AL2(61,0)
         DC    A(T0042-COBBEG),AL2(62,0)
         DC    A(T0043-COBBEG),AL2(64,0)
         DC    A(T0044-COBBEG),AL2(65,0)
         DC    A(T0045-COBBEG),AL2(66,0)
         DC    A(T0046-COBBEG),AL2(67,0)
         DC    A(T0047-COBBEG),AL2(67,0)
         DC    A(T0048-COBBEG),AL2(68,0)
         DC    A(T0049-COBBEG),AL2(69,0)
         DC    A(T0050-COBBEG),AL2(71,0)
         DC    A(T0051-COBBEG),AL2(72,0)
         DC    A(T0052-COBBEG),AL2(72,0)
...
D0001    DS    0CL80               K-REC (01 group)
D0002    DC    CL10' '             K-KEY PIC X(10)
D0003    DC    CL5' '              K-TEXT PIC X(5)
D0004    DC    CL65' '             FILL0004 PIC X(65)
D0005    DC    CL2'00'             K-STATUS PIC X(2)
         DS    XL6                 reserve the rest of a table
D0006    DC    CL1'N'              EOF PIC X(1)
*---------------------------------------------------------------
