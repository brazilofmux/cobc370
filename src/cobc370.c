...
typedef struct {
    ...
    int   nrep;
    struct { char from[120], to[120]; int pseudo, qfrom, qto; } rep[16];
} Src;
...
static void copy_replace(Src *s)
{
    ...
    while (*p) {
        ...
        for (int k = 0; k < s->nrep && !hit; k++) {
            ...
            if (s->rep[k].qfrom) {
                ...
                if ((q == '\'' || q == '"') && !strncasecmp(p + 1, from, fl) && p[1 + fl] == q)
                    { hit = 1; nxt = p + 1 + fl + 1; qch = q; }
            } else if (s->rep[k].pseudo) {
            ...
            if (s->rep[k].qto) {
                if (!qch) qch = '\'';
                size_t tl = strlen(to);
                if ((size_t)(o - out) + tl + 2 >= sizeof out - 1)
                    die("COPY REPLACING made a line too long");
                *o++ = qch; memcpy(o, to, tl); o += tl; *o++ = qch;
            } else {
                size_t tl = strlen(to);
                if ((size_t)(o - out) + tl >= sizeof out - 1)
                    die("COPY REPLACING made a line too long");
                memcpy(o, to, tl); o += tl;
            }
            p = nxt; changed = 1;
        ...
        if (!hit) {
            if ((size_t)(o - out) >= sizeof out - 1) die("COPY REPLACING made a line too long");
            *o++ = *p++;
        }
    }
    *o = 0;
...
static int src_fill(Src *s)
{
    for (;;) {
        ...
        /* A D in the indicator area is a debugging line. Debug is the null
         * level here, so those lines are comments -- I-106, without
         * WITH DEBUGGING MODE. */
        if (s->buf[6] == '*' || s->buf[6] == '/' || s->buf[6] == 'D' || s->buf[6] == 'd') continue;
...
static void scan_token(void)
{
    ...
    for (;;) {
        ...
        if (lex_parens && !lex_picture && (*src.p == '(' || *src.p == ')')) break;
        if (*src.p == '.') {
...
typedef struct {
    ...
    int  bwz;         /* BLANK WHEN ZERO: a zero value is stored as spaces, II-14 */
...
/* ---- Report Writer -----------------------------------------------------
 * The standard's Report Writer Control System, generated per report. Each
 * report carries a state block: LINE-COUNTER and PAGE-COUNTER as real data
 * items (so SOURCE and the Procedure Division can name them), a pending
 * carriage-control byte, whether a body group has been presented on the
 * current page, and whether the first GENERATE has happened. Each report
 * group compiles to a renderer that carries exactly the presentation rules
 * its row of the tables in VIII 2.5.5 names -- the row is fixed by the
 * group's TYPE and the shape of its LINE sequence, both known here.
 *
 ...
 * into the print buffer at its column, then hand the buffer to COBWRL, which
 * positions the paper and writes.
 *
 * Implemented so far: PAGE HEADING and DETAIL groups, the PAGE clause with
...
typedef struct { int column, sym, src; Node *sub; char lit[MAXTOK]; int litlen;
                 char pend_name[31], pend_qual[31];            /* src -2: resolved at the section's end */
                 int gind; } RField;                            /* GROUP INDICATE */
typedef struct { int absolute, n, next_page, first_fld, nfld; } RLine;
typedef struct { char name[31]; int report, type, first_line, nline, ng_kind, ng_n, ctl_level, name_sym;
                 int has_gind; } RGroup;                         /* a GROUP INDICATE item: RGI cell */
...
typedef struct {
    ...
    int  has_code; char code[3];        /* the CODE clause, VIII 2.7 */
    char lbl_blank[9];                  /* this report's blank line, with its CODE */
} Report;
...
typedef struct {
    ...
    int  group;         /* USE BEFORE REPORTING: the report group, else -1 */
} Decl;
...
static int resolve_sym_quiet(const char *name, char q[][31], int nq)
{
    ...
    if (found < 0 && !nq && !strcmp(name, "CURRENT-DATE")) {
        int cur = wslen;
        make_curdate(&cur);
        return curdate_sym;
    }
...
static int resolve_sym(const char *name, char q[][31], int nq)
{
    ...
    char m[160];
    if (count == 0) {
        if (!nq && !strcmp(name, "CURRENT-DATE")) {
            /* A special register, created on first use so a mention in COPY
             * text still finds it. Storage is claimed at the end of WS. */
            int cur = wslen;
            make_curdate(&cur);
            return curdate_sym;
        }
...
static void copy_operand(char *out, size_t on, int *pseudo, int *quoted)
{
    *quoted = 0;
    for (;;) {
        while (*src.p == ' ') src.p++;
    ...
    *pseudo = 0;
    scan_token();
    if (tok.literal) { *quoted = 1; snprintf(out, on, "%s", tok.text); }
...
static void copy_statement(void)
{
    ...
    if (is("REPLACING")) {
        for (;;) {
            ...
            int ps, qf, qt;
            copy_operand(cb.rep[cb.nrep].from, sizeof cb.rep[0].from, &ps, &qf);
            cb.rep[cb.nrep].pseudo = ps;
            cb.rep[cb.nrep].qfrom = qf;
            scan_token();
            if (!is("BY")) die("COPY REPLACING wants  operand BY operand");
            copy_operand(cb.rep[cb.nrep].to, sizeof cb.rep[0].to, &ps, &qt);
            cb.rep[cb.nrep].qto = qt;
            cb.nrep++;
 ...
 * PAGE-COUNTER OF report-name resolves the usual way. Only the RWCS may set
 * LINE-COUNTER; PAGE-COUNTER the program may alter. */
static int make_counter(const char *name, int gparent)
{
    if (nsym >= MAXSYM) die("too many data items");
    Sym *sy = &syms[nsym];
...
static void parse_report_section(void)
{
    ...
    /* SOURCE identifiers may be subscripted, so parentheses are tokens here;
     * PICTURE strings are scanned by next_pic and do not notice. */
    int saved_parens = lex_parens;
    lex_parens = 1;
    ...
    while (!tok.eof && !is("PROCEDURE") && !is("WORKING-STORAGE")) {
        if (is("RD")) {
            ...
            Report *rp = &reports[cur_report];
            {
                ...
                int gi = nsym++;
                rp->lc_sym = make_counter("LINE-COUNTER", gi);
                rp->pc_sym = make_counter("PAGE-COUNTER", gi);
            }
            while (!tok.eof && !is(".")) {
                if (is("PAGE")) {
                    next();
                    if (is("LIMIT") || is("LIMITS")) next();
                    if (is("IS") || is("ARE")) next();
                    rp->has_page = 1;
                    ...
                    /* VIII 2.7: a two-character literal in the first two
                     * character positions of every record of the report,
                     * not counted in the print line but in the record. */
                    next();
                    if (!tok.literal || tok.len != 2) die("CODE takes a two-character nonnumeric literal -- VIII 2.7.3(1)");
                    rp->has_code = 1; memcpy(rp->code, tok.text, 2); rp->code[2] = 0;
                    next();
                }
            ...
            expect(".");
            if (rp->has_page) {
                /* The implicit values of VIII 2.16.4(2). */
                if (!rp->heading) rp->heading = 1;
                if (!rp->first_detail) rp->first_detail = rp->heading;
                if (!rp->last_detail && !rp->footing) rp->last_detail = rp->footing = rp->page_limit;
                else if (!rp->last_detail) rp->last_detail = rp->footing;
                else if (!rp->footing) rp->footing = rp->last_detail;
                if (rp->first_detail < rp->heading || rp->last_detail < rp->first_detail
                    || rp->footing < rp->last_detail || rp->page_limit < rp->footing)
                    die("the PAGE clause's phrases must not decrease: HEADING, FIRST DETAIL, "
                        "LAST DETAIL, FOOTING, then the page limit -- VIII 2.16.3");
            } else if (rp->heading || rp->first_detail || rp->last_detail || rp->footing)
                die("HEADING, FIRST DETAIL, LAST DETAIL and FOOTING belong to the PAGE clause");
            continue;
        }
            ...
            next_group:
                ...
                while (!tok.eof && !is(".") && !is("UPON") && !is("RESET") && !is("COLUMN")
                    ...
                    if (is("OF") || is("IN")) {
                        next();
                        if (strlen(tok.text) > 30) die("data name too long");
                        strcpy(sm.op_qual[sm.nops], tok.text); next();
                    }
                    if (is("(")) {
                ...
                next(); if (is("INDICATE")) next();
                gind = 1;
            } else if (is("JUSTIFIED") || is("JUST")) {
                next(); if (is("RIGHT")) next();
                just = 1;
            } else if (is("BLANK")) {
                next(); if (is("WHEN")) next();
                if (!is("ZERO") && !is("ZEROS") && !is("ZEROES")) die("expected ZERO after BLANK WHEN");
                next();
                bwz = 1;
        ...
        if (has_col || pic[0] || src != -1 || litlen >= 0) {
            ...
            if (gind) {
                if (rgroups[cur_group].type != RG_DETAIL)
                    die("GROUP INDICATE may appear only in a DETAIL group -- VIII 2.12.3(1)");
                fl->gind = 1; rgroups[cur_group].has_gind = 1;
            }
            ...
            sy->bytes = pi.bytes;
            sy->elem = sy->bytes;
            if (just) { if (!pi.is_alpha) die("JUSTIFIED needs an alphabetic or alphanumeric item"); sy->just = 1; }
            if (bwz)  { if (pi.is_alpha) die("BLANK WHEN ZERO needs a numeric or numeric edited item"); sy->bwz = 1; }
...
static void parse_data_division(void)
{
    ...
    else if (is("LINKAGE")) { next(); expect("SECTION"); expect("."); in_linkage = 1; }
    ...
    while (!tok.eof && !is("PROCEDURE")) {
        ...
        if (is("FD")) {
            ...
            while (!tok.eof && !is(".")) {
                if (is("REPORT") || is("REPORTS")) {
                    next(); if (is("IS") || is("ARE")) next();
                    while (!tok.eof && !is(".") && !is("LABEL") && !is("BLOCK") && !is("RECORD")
                           && !is("DATA") && !is("VALUE") && !is("CODE-SET") && !is("RECORDING")
                           && !is("LINAGE")) {
                        if (nreport >= MAXREPORT) die("too many reports");
                        Report *rp = &reports[nreport];
                        memset(rp, 0, sizeof *rp);
                        snprintf(rp->name, sizeof rp->name, "%s", tok.text);
                        snprintf(rp->lbl_ctl,  sizeof rp->lbl_ctl,  "RCTL%03d",  nreport);
                        snprintf(rp->lbl_body, sizeof rp->lbl_body, "RBODY%03d", nreport);
                        ...
                        snprintf(rp->lbl_blank, sizeof rp->lbl_blank, "RBLK%03d", nreport);
                        ...
                        files[cur_file].report = nreport;    /* the last one; any marks the file */
                        nreport++;
                        next();
                    }
                    continue;
        ...
        if (level == 88) {
            ...
            for (int k = 0; k < nsym; k++)
                if (!strcmp(syms[k].name, cn->name) && syms[k].gparent == cn->gparent)
            ...
            for (;;) {
                ...
                if (nsym != first) {
                    if (nsym >= MAXSYM) die("too many data items");
                    memset(e, 0, sizeof *e);
                    e->level = 88; e->is_88 = 1; e->parent = p; e->gparent = p; e->c88_next = -1;
        ...
        while (!tok.eof && !is(".")) {
            if (is("PIC") || is("PICTURE")) {
                 ...
                 * subordinate to it takes the same offsets. II-27: the
                 * redefined name is a preceding item at this level, so an
                 * unqualified name takes the nearest earlier one in this
                 * group -- not the first KIND in the whole program. */
                next();
                char rnm[31], rq[MAXQUAL][31];
                if (strlen(tok.text) > 30) die("data name too long");
                snprintf(rnm, sizeof rnm, "%s", tok.text);
                next();
                int rnq = consume_quals(rq);
                int t = -1;
                if (rnq) t = resolve_sym(rnm, rq, rnq);
                else {
                    for (int k = nsym - 1; k >= 0; k--)
                        if (!strcmp(syms[k].name, rnm) && syms[k].level == level
                            && syms[k].gparent == sy->gparent) { t = k; break; }
                    if (t < 0) t = resolve_sym(rnm, rq, 0);
                }
                ...
                /* BLANK WHEN ZERO, II-14: when the value stored is zero the
                 * item is filled with spaces. Numeric or numeric edited,
                 * USAGE DISPLAY only (syntax rule 1). */
                next(); if (is("WHEN")) next();
                if (!is("ZERO") && !is("ZEROS") && !is("ZEROES")) die("expected ZERO after BLANK WHEN");
                next();
                sy->bwz = 1;
                ...
                sy->usage = U_COMP; sy->is_index = 1; saw_usage = 1; next();
            }
        ...
        /* USAGE and SIGN on a group apply to every elementary item that does
         * not carry its own -- II-17, II-31. Walk enclosing groups, inner
         * first, so a nested USAGE wins. */
        if (!saw_usage || !saw_sign) {
            for (int k = sp - 1; k >= 0; k--) {
                const Sym *g = &syms[stack[k]];
                if (!saw_usage && (g->usage != U_DISPLAY || g->is_index)) {
                    sy->usage = g->usage;
                    sy->is_index = g->is_index;
                    saw_usage = 1;
                }
                if (!saw_sign && (g->sgn_lead || g->sgn_sep)) {
                    sy->sgn_lead = g->sgn_lead;
                    sy->sgn_sep = g->sgn_sep;
                    saw_sign = 1;
                }
            }
        }
...

        {
            ...
            if (pi.is_alpha) {
                ...
                if (sy->has_value == 2)
                    die("VALUE SPACES is alphanumeric; a numeric item takes ZERO");
                if (sy->has_value == 3)
                ...
                case U_DISPLAY:
                    ...
                    /* P occupies no character position -- II-21. pi.bytes is
                     * the stored width; pi.digits counts P as well. */
                    sy->bytes = pi.bytes + (sy->sgn_sep ? 1 : 0);
                    break;
                case U_COMP3:
                    if (sy->sgn_lead || sy->sgn_sep)
                        die("the SIGN clause applies only to USAGE DISPLAY -- "
                            "syntax rule 2 on II-31");
                    sy->bytes = pi.bytes / 2 + 1; break;
                case U_COMP:
            ...
            if (sy->bwz && sy->usage != U_DISPLAY)
                die("BLANK WHEN ZERO applies only to USAGE DISPLAY -- syntax "
                    "rule 1 on II-14");
        }
        sy->elem = sy->bytes;
        if (sy->occurs > 0) {
    ...
    for (int i = 0; i < nfile; i++) {
        if (files[i].report >= 0) {
            /* ASA byte, the CODE when any report on the file has one (all
             * must then, 2.7.3(2)), and 132 columns. */
            int code = 0, plain = 0;
            for (int r = 0; r < nreport; r++)
                if (reports[r].file == i) { if (reports[r].has_code) code = 1; else plain = 1; }
            if (code && plain) die("CODE must be given for all the reports of a file or none -- VIII 2.7.3(2)");
            files[i].reclen = code ? 135 : 133;
            continue;
        }
...
static int nlabel;
static int next_sentence_lab;   /* NEXT SENTENCE: branch here when the period lands */
...
static void eat_period(void)
{
    ...
    next();
    at_period = 1;
    if (next_sentence_lab) {
        new_stmt(ST_LABEL)->dst = next_sentence_lab;
        next_sentence_lab = 0;
    }
}
...
static void parse_one_statement(void)
{
    ...
    if (is("INITIATE") || is("TERMINATE")) {
        ...
        if (!n) die("INITIATE/TERMINATE needs a report-name");
        eat_period();
        return;
    }

    if (is("SUPPRESS")) {
        /* SUPPRESS PRINTING, VIII 3.3: inside a USE BEFORE REPORTING
         * procedure, the group about to be produced is not. */
        next(); if (is("PRINTING")) next();
        if (decl_end_para >= 0) die("SUPPRESS may appear only in a USE BEFORE REPORTING procedure -- VIII 3.3.3(1)");
        new_stmt(ST_SUPPRESS);
        eat_period();
        return;
    }
    ...
    if (is("GENERATE")) {
        ...
        next();
        if (is("OF") || is("IN")) {          /* qualified by the report-name */
            next();
            if (report_index(tok.text) != rgroups[g].report) die("GENERATE: the qualifier must be the group's report");
            next();
        }
        eat_period();
        return;
    ...
    if (is("NEXT")) {
        /* II-66: skip to the period that ends this sentence. The label is
         * planted when that period is eaten, so a NEXT SENTENCE inside IF
         * lands after the ELSE as well. */
        next();
        expect("SENTENCE");
        if (!next_sentence_lab) next_sentence_lab = ++nlabel;
        new_stmt(ST_BRANCH)->dst = next_sentence_lab;
        eat_period();
        return;
    }
...
static void parse_procedure(void)
{
        ...
        while (!is(".")) {
            ...
            if (!syms[i].linkage)
                die("PROCEDURE DIVISION USING names an item that is not in the "
                    "LINKAGE SECTION");
            if (syms[i].level != 1 && syms[i].level != 77)
                die("PROCEDURE DIVISION USING needs a level 01 or 77 item");
            using_parm[nusing++] = i;
        }
    }
    expect(".");
    if (is("DECLARATIVES")) {
        ...
        while (!tok.eof && !is("END")) {
            ...
            if (is("BEFORE")) {
                /* USE BEFORE REPORTING identifier, VIII 3.5: run just before
                 * the named report group is produced. */
                next(); expect("REPORTING");
                Decl *d = &decls[ndecl];
                memset(d, 0, sizeof *d);
                if (npara == 0 || !paras[npara-1].is_section)
                    die("USE must follow a section header in the declaratives");
                d->sect = npara - 1;
                d->group = rgroup_index(tok.text);
                if (d->group < 0) die("USE BEFORE REPORTING must name a report group");
                for (int k = 0; k < ndecl; k++)
                    if (decls[k].group == d->group) die("a report group may appear in only one USE statement -- VIII 3.5.3(2)");
                next();
                expect(".");
                ndecl++;
                continue;
            ...
            else die("USE ... EXCEPTION or ERROR PROCEDURE, or USE BEFORE REPORTING, "
                     "are the forms this compiler implements");
...
/* LH/L/STH/ST need the field on a halfword or fullword. Interiors are packed
 * with no slack, so a COMP after a PIC X sits at offset 1 and those RX
 * instructions take a specification exception. MVC through DWK, which is
 * aligned, is the byte-oriented way to the same register. */
static int comp_rx_aligned(const Sym *sy)
{
    return sy->elem > 0 && sy->offset % sy->elem == 0;
}

static void gen_comp_load_r(const Sym *sy, Node *sub, int reg)
{
    char b[96], f[64];
    if (comp_rx_aligned(sy)) {
        ...
        field_ref_m(sy, sub, FR_SS_NOLEN, sy->elem, 7, f, sizeof f);
        snprintf(b, sizeof b, "DWK(%d),%s", sy->elem, f);
        asm_line("", "MVC", b, "COMP, not on its boundary");
        snprintf(b, sizeof b, "%d,DWK", reg);
        asm_line("", sy->elem == 2 ? "LH" : "L", b, "");
    }
}
...
static void gen_comp_store_r(const Sym *sy, Node *sub, int reg)
{
    ...
    if (comp_rx_aligned(sy)) {
        ...
        snprintf(b, sizeof b, "%d,DWK", reg);
        asm_line("", sy->elem == 2 ? "STH" : "ST", b, "");
        field_ref_m(sy, sub, FR_SS_LEN, sy->elem, 6, f, sizeof f);
        snprintf(b, sizeof b, "%s,DWK", f);
        asm_line("", "MVC", b, "COMP, not on its boundary");
    }
}
...
static void gen_file_status(const File *f, const char *code)
{
...
{
    char b[128], fd[64];
    int has_st = f->status_sym >= 0;
    if (!has_st && gen_use_decl < 0) return;
    const Sym *st = has_st ? &syms[f->status_sym] : NULL;
...
/* BLANK WHEN ZERO, II-14: after the store, a zero value becomes spaces. The
 * test is on the packed value being stored, which is still in op. */
static void gen_blank_when_zero(const Sym *sy, Node *sub, const char *op)
{
    char b[128], f[64], lz[16];
    snprintf(lz, sizeof lz, "L%04d", ++genlabel);
    snprintf(b, sizeof b, "%s,%s+15(1)", op, intern_const("0"));
    asm_line("", "CP", b, "BLANK WHEN ZERO: is it zero?");
    asm_line("", "BNE", lz, "");
    field_ref(sy, sub, sy->elem, 6, f, sizeof f);
    snprintf(b, sizeof b, "%s,%s", f, intern_str(" ", 1, sy->elem));
    asm_line("", "MVC", b, "then spaces");
    asm_line(lz, "DS", "0H", "");
}
...
static void gen_store_op(const Sym *sy, Node *sub, const char *op)
{
    ...
    if (sy->edited || (sy->usage == U_DISPLAY && (sy->digits > 16 || sy->sgn_lead || sy->sgn_sep))) {
        ...
        if (sy->bwz) gen_blank_when_zero(sy, sub, "PWK1(16)");
        return;
    }
    if (sy->bwz) {
        /* For a non-edited item the spaces ARE the stored value, so skip the
         * UNPK/ZAP/ST when the packed source is zero. Edited items overlay
         * the picture after the store; blanking those stays after it. */
        char lz[16], ls[16];
        snprintf(lz, sizeof lz, "L%04d", ++genlabel);
        snprintf(ls, sizeof ls, "L%04d", ++genlabel);
        snprintf(b, sizeof b, "%s,%s+15(1)", op, intern_const("0"));
        ...
        case U_COMP3:
            ...
            break;
        default:
            die("BLANK WHEN ZERO applies only to USAGE DISPLAY");
        }
        asm_line(lz, "DS", "0H", "");
        return;
    }
...
static const char *intern_str(const char *text, int len, int pad)
{
    ...
    if (n >= MAXSCONST) die("literal too long");
...
static int gen_expr(Node *n, int d, int tgtscale, int want)
{
    ...
    case N_MUL: {
         ...
         * wider one on the left when the other fits, otherwise refuse. */
        int swap = 0;
        if (L2 > 8) {
            int Ll = pk_bytes(pl);
            if (Ll <= 8) swap = 1;
            else die("a multiplier of more than 15 digits is not implemented");
        }
        if (swap) {
            int L1 = pk_bytes(pr) + pk_bytes(pl);
            if (L1 > 16) L1 = 16;
            if (L < L1) L = L1;
            gen_expr(n->r, d, tgtscale, L);
            if (!direct_operand(n->l, sl, 1, 8, op, sizeof op, &oplen)) {
                gen_expr(n->l, d + 1, tgtscale, 1);
                tail_op(wk2, pk_bytes(pl) > 8 ? 8 : pk_bytes(pl), op, sizeof op);
            }
            tail_op(wk, L, t, sizeof t);
            snprintf(b, sizeof b, "%s,%s", t, op);
...
static void emit_runtime(void)
{
    ...
    asm_line("", "L", "11,24(0,1)", "A(this report's blank line)");
    ...
    asm_line("", "BNE", "COBW010", "");
    asm_line("", "MVI", "0(9),C' '", "once");
    asm_line("", "CH", "7,RWONE", "to line 1?");
    asm_line("", "BNE", "COBW005", "");
    asm_line("", "MVI", "0(5),C'1'", "the line itself ejects");
    asm_line("", "SR", "6,6", "");
    asm_line("", "B", "COBW020", "");
    asm_line("COBW005", "MVI", "0(11),C'1'", "the blank line ejects");
    asm_line("", "PUT", "(2),(11)", "a blank line at the top of a new page");
    asm_line("", "MVI", "0(11),C' '", "");
    asm_line("", "LA", "6,1", "");
    ...
    asm_line("", "SR", "6,6", "no hole yet");
    asm_line("DCA010", "CLC", "0(8,4),0(2)", "this one?");
    asm_line("", "BE", "DCA030", "already loaded");
    asm_line("", "CLI", "0(4),X'00'", "an empty entry?");
    asm_line("", "BNE", "DCA020", "");
    asm_line("", "LTR", "6,6", "");
    asm_line("", "BNZ", "DCA020", "keep the first hole");
    asm_line("", "LR", "6,4", "");
    asm_line("DCA020", "LA", "4,12(4)", "");
    asm_line("", "BCT", "5,DCA010", "");
    asm_line("", "LTR", "6,6", "a hole to remember this load?");
    asm_line("", "BZ", "DCAFULL", "table full");
    asm_line("", "LR", "4,6", "");
    asm_line("", "B", "DCA050", "");
    asm_line("DCAFULL", "LOAD", "EPLOC=(2)", "table full: load without remembering");
    asm_line("", "LR", "15,0", "");
    asm_line("", "B", "DCA040", "");
    ...
    asm_line("", "BE", "CANNEXT", "empty: keep looking");
    asm_line("", "CLC", "0(8,4),0(2)", "");
    asm_line("", "BE", "CAN020", "");
    asm_line("CANNEXT", "LA", "4,12(4)", "");
...
static void emit_move(const Sym *d, Node *dsub, const Sym *sv, Node *ssub)
{
    ...
    if (sv->usage == U_DISPLAY && d->usage == U_DISPLAY && sv->scale == d->scale
        && !d->edited && !sv->edited && !d->is_signed && !sv->is_signed && !d->bwz
...
/* The report's counters, addressed: LINE-COUNTER or PAGE-COUNTER. */
static const char *rw_counter(const Report *rp, int page)
{
    const Sym *sy = &syms[page ? rp->pc_sym : rp->lc_sym];
    need_sym_base(sy);
    return sy->label;
}
 ...
 * with the target line already in R2. A GROUP INDICATE item is placed only
 * while its group's indicate cell is set (VIII 2.12); the CODE, when there
 * is one, takes the two positions after the carriage control. */
static void emit_report_line(int gi, const RLine *l)
{
    char b[160], lg[16];
    const RGroup *g = &rgroups[gi];
    const Report *rp = &reports[g->report];
    int off = rp->has_code ? 3 : 1;             /* where column 1 is */
    asm_line("", "ST", "2,RTGT", "");
    asm_line("", "MVI", "RBUF+1,C' '", "");
    asm_line("", "MVC", "RBUF+2(133),RBUF+1", "blank the line");
    if (rp->has_code) {
        snprintf(b, sizeof b, "RBUF+1(2),%s", intern_str(rp->code, 2, 2));
        asm_line("", "MVC", b, "the CODE");
    }
    for (int fi = l->first_fld; fi < l->first_fld + l->nfld; fi++) {
        RField *f = &rfields[fi];
        const Sym *fs = &syms[f->sym];
        if (f->gind) {
            snprintf(lg, sizeof lg, "L%04d", ++genlabel);
            snprintf(b, sizeof b, "RGI%03d,X'00'", gi);
            asm_line("", "CLI", b, "GROUP INDICATE: the first time since a break or a page?");
            asm_line("", "BE", lg, "no: spaces");
        }
        if (f->src >= 0) {
            emit_move(fs, NULL, &syms[rw_source_sym(gi, f->src)], f->sub);
            reset_bases();
            need_sym_base(fs);
            snprintf(b, sizeof b, "RBUF+%d(%d),%s", off + f->column - 1, fs->bytes, fs->label);
            asm_line("", "MVC", b, "COLUMN placement");
        } else {
            const char *sl = intern_str(f->lit, f->litlen, fs->bytes);
            snprintf(b, sizeof b, "RBUF+%d(%d),%s", off + f->column - 1, fs->bytes, sl);
            asm_line("", "MVC", b, "COLUMN literal");
        }
        if (f->gind) { asm_line(lg, "DS", "0H", ""); reset_bases(); }
    }
 ...
 * line printed. */
static int rw_use_nret = 9000;     /* return labels R9001..., apart from the statement ones */
...
static void emit_report_group(int gi)
{
    ...
    asm_line(lab, "ST", b, "save the return");
    reset_bases();
    /* USE BEFORE REPORTING, then SUPPRESS: VIII 2.21.4(9)a-b, (10)c-d,
     * (11)b-c. A suppressed group presents nothing, moves no paper and
     * leaves LINE-COUNTER alone; its footing's resets still follow. */
    char lsup[16]; lsup[0] = 0;
    for (int d = 0; d < ndecl; d++) {
        if (decls[d].group != gi) continue;
        asm_comment(" USE BEFORE REPORTING");
        gen_call_range(decls[d].sect, section_end(decls[d].sect), &rw_use_nret);
        snprintf(lsup, sizeof lsup, "L%04d", ++genlabel);
        asm_line("", "CLI", "RSUPPR,X'01'", "SUPPRESS PRINTING?");
        asm_line("", "BE", lsup, "");
    }
    if (g->nline == 0) {
        if (lsup[0]) asm_line(lsup, "DS", "0H", "");
        asm_line("", "MVI", "RSUPPR,X'00'", "");
        asm_line("", "L", b, ""); asm_line("", "BR", "14", "not printable");
        return;
    }
    ...
    for (int li = g->first_line; li < g->first_line + g->nline; li++) {
        ...
        if (l->absolute) {
            ...
            asm_line("", "B", l3, "");
            asm_line(l2, "DS", "0H", "");
            snprintf(b, sizeof b, "2,%s", intern_full(rp->first_detail));
            asm_line("", "C", b, "the heading ran past FIRST DETAIL?");
            asm_line("", "BNL", "*+12", "");
            snprintf(b, sizeof b, "2,%d", rp->first_detail);
            asm_line("", "LA", b, "no: the first line is FIRST DETAIL");
            asm_line("", "B", l3, "");
    ...
    if (g->has_gind) {
        snprintf(b, sizeof b, "RGI%03d,X'00'", gi);
        asm_line("", "MVI", b, "GROUP INDICATE items shown; not again until a break or a page");
    }
    if (body) {
...
/* Set every GROUP INDICATE cell of a report: after INITIATE, a page advance
 * or a control break the next presentation of each DETAIL shows them. */
static void emit_set_indicate(int ri)
{
    char b[64];
    for (int k = 0; k < nrgroup; k++) {
        if (rgroups[k].report != ri || !rgroups[k].has_gind) continue;
        snprintf(b, sizeof b, "RGI%03d,X'01'", k);
        asm_line("", "MVI", b, "GROUP INDICATE items show next time");
    }
}
...
static void emit_report_advance(int ri)
{
    char b[160];
    Report *rp = &reports[ri];
    snprintf(b, sizeof b, " page advance, report %s", rp->name);
    asm_comment(b);
...
static void generate(void)
{
    ...
    for (int i = 0; i < nstmt; i++) {
        ...
        case ST_SUPPRESS:
            asm_comment(" SUPPRESS PRINTING");
            asm_line("", "MVI", "RSUPPR,X'01'", "the group being produced is not presented");
            break;
        case ST_TERMINATE: {
        ...
        case ST_GENERATE: {
            ...
            snprintf(b, sizeof b, "%s,X'00'", rp->lbl_gen);
            asm_line("", "CLI", b, "the first GENERATE?");
            asm_line("", "BNE", lnf, "");
            ...
            if (rp->nctl) {
                ...
                asm_line(lbrk, "DS", "0H", "");
                reset_bases();
                emit_set_indicate((int)(rp - reports));
                emit_control_groups(rp, 1);
                emit_save_controls(rp);
                emit_control_groups(rp, 0);
        ...
        case ST_WRITE: {
            ...
            if (st->src >= 0) {
                ...
                gen_move_alpha(wrec, NULL, sv, st->ssub);
            }
            if (f->vsam) {
        ...
        case ST_CALL: {
            ...
            for (int k = 0; k < st->ndop; k++) {
                ...
                char fa[64];
                need_sym_base(a);
                field_ref_m(a, st->dop[k].sub, FR_RX, a->bytes, 6, fa, sizeof fa);
        ...
        case ST_SUB: {
            ...
            else if (st->imm == 2) snprintf(b, sizeof b, " %s %s -> %s", verb, st->lit, d->name);
            ...
            if (st->op == ST_MOVE) {
                ...
                if (st->imm == 2 && !(d->is_alpha || d->is_group)) {
                    ...
                    if (d->usage != U_DISPLAY || d->is_signed || d->scale != 0 ||
                        ...
                        die("MOVE of a nonnumeric literal to that numeric item "
                            ...
                            "integer");
                    for (int k = 0; k < st->litlen; k++)
                        if (!isdigit((unsigned char)st->lit[k]))
                    ...
                    char pad[MAXSCONST];
                    if (sn >= dn) memcpy(pad, st->lit + (sn - dn), dn);
                    else {
                        memset(pad, '0', dn - sn);
                        memcpy(pad + dn - sn, st->lit, sn);
                    }
                ...
                if (st->imm == 2) {
                    ...
                    } else sl = intern_str(st->lit, st->litlen, dn);
                    char fd[64];
                    field_ref(d, st->dsub, dn, 6, fd, sizeof fd);
                    snprintf(b, sizeof b, "%s,%s", fd, sl);
                ...
                if (st->imm && alldig && d->usage == U_DISPLAY && !d->edited && !d->is_signed && !d->bwz
                ...
                if (d->usage == U_COMP && !d->edited && ws == d->scale) {
                    ...
                    if (!st->imm && syms[st->src].usage == U_COMP && syms[st->src].scale == d->scale) {
                        const Sym *sv = &syms[st->src];
                        need_sym_base(d); need_sym_base(sv);
                        gen_comp_load_r(d, st->dsub, 2);
                        if (comp_rx_aligned(sv)) {
                            field_ref(sv, st->ssub, 0, 7, fop, sizeof fop);
                            snprintf(b, sizeof b, "2,%s", fop);
                            asm_line("", sv->elem == 2 ? (st->op == ST_ADD ? "AH" : "SH")
                                                       : (st->op == ST_ADD ? "A"  : "S"),
                                     b, "binary, same scale: in the register");
                        } else {
                            gen_comp_load_r(sv, st->ssub, 3);
                            asm_line("", st->op == ST_ADD ? "AR" : "SR", "2,3",
                                     "binary, same scale: in the register");
                        }
                        gen_comp_store_r(d, st->dsub, 2);
                        break;
                    ...
                    if (op) {
                        ...
                        gen_comp_load_r(d, st->dsub, 2);
                        snprintf(b, sizeof b, "2,%s", fop);
                        asm_line("", op, b, "binary, same scale: in the register");
                        gen_comp_store_r(d, st->dsub, 2);
                        break;
                    }
                }
    ...
    if (nreport) {
        ...
        for (int i = 0; i < nrgroup; i++) {
            ...
            snprintf(b, sizeof b, "X'80',AL3(%s)", rp->has_code ? rp->lbl_blank : "RTBLNK");
            asm_line("", "DC", b, "the blank line; last parameter");
            snprintf(lab, sizeof lab, "RGS%03d", i);
            asm_line(lab, "DS", "F", "return address");
            if (rgroups[i].has_gind) {
                snprintf(lab, sizeof lab, "RGI%03d", i);
                asm_line(lab, "DC", "X'01'", "GROUP INDICATE: show them");
            }
        }
        asm_line("RSUPPR", "DC", "X'00'", "SUPPRESS PRINTING was executed");
        for (int i = 0; i < nreport; i++) {
            if (reports[i].lc_sym < 0) continue;
            ...
            if (reports[i].has_code) {
                snprintf(b, sizeof b, "C' ',C'%s',CL132' '", reports[i].code);
                asm_line(reports[i].lbl_blank, "DC", b, "a blank line with the CODE");
            }
        ...
        asm_line("RBUF", "DC", "CL135' '", "ASA byte, room for a CODE, 132 columns");
    }
