...
| QSAM | sequential read, write, rewrite; fixed and variable-length records (`RECORDING MODE`, `RECORD CONTAINS m TO n`), blocked and unblocked, `BLOCK CONTAINS 0`; `OPTIONAL`, `EXTEND`, `WRITE ... ADVANCING` exactly as IBM's compiler writes it (the record's first byte, machine carriage control), `LINAGE` with `END-OF-PAGE`, `USE` declaratives |
...
## Layout
    ...
    tests/   131 COBOL programs and their oracles
